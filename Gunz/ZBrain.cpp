...
bool ZBrain::FindTarget()
{
	...
	for (auto itor = ZGetGame()->GetCharacterMgr()->begin(); itor != ZGetGame()->GetCharacterMgr()->end(); ++itor)
	{
...
void ZBrain::ProcessBuildPath( float fDelta)
{
	...
	if ( ( m_Behavior.GetOffenseType() == ZOFFENSETYPE_RANGE) || m_Behavior.IsFriendly())
	{
		...
		if ( bStop)
		{
			...
			if (m_pBody->CanSee(pTarget) && m_pBody->CanAttackRange(pTarget) && m_pBody->GetTeamID() != pTarget->GetTeamID())
			{
				m_pBody->Stop();
				m_pBody->GetTaskManager()->Clear();
