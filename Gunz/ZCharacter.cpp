...
void ZCharacter::CheckDrawWeaponTrack()
{
	...
	if( ( m_pVMesh->GetSelectWeaponMotionType() == eq_wd_katana ) || 
		...
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_katana1	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_katana2	) || 
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_katana3	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_katana4	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_axe		) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_baseball) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_fighter	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_sword2	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_blade2	) ||
		(m_pVMesh->GetSelectWeaponMotionType() == eq_wd_blade3	))
	{
...
//	무기타잎별로 하나만 등록되어 있으니까 기본공격 모션의 길이를 기준으로 한다..
	...
	switch(pSelectItemDesc->m_nWeaponType.Ref())
	{
		...
		case MWT_DOUBLE_KATANA2:
		case MWT_DOUBLE_KATANA3:
			nReturnDelay -= 299;
			break;
...
float ZCharacter::GetMoveSpeedRatio()
{
	...
	{
#ifdef _COWBOY_SKILL
		if (ZGetMyInfo()->GetClassJob() == MMCJ_COWBOY)
		{
			fRatio = 100 / 97.f; // Idunno i think its x1 :D
		}
		else
#endif
...
void ZCharacter::OnChangeWeapon(char* WeaponModelName)
{
	...
	if( m_pVMesh ) {
		...
		if( (eq_wd_katana == type) ||(eq_wd_katana1 == type) || (eq_wd_katana2 == type) || (eq_wd_katana3 == type) || (eq_wd_axe == type) || (eq_wd_baseball == type) || (eq_wd_fighter == type) )
		{
#ifdef _BIRDSOUND
		...
		else if( ( eq_wd_sword == type ) || (eq_wd_sword2 == type ) )
		{
#ifdef _BIRDSOUND
			ZGetSoundEngine()->PlaySoundCharacter("fx_dagger_sheath",GetPosition(),IsObserverTarget());
#else
		...
		else if( eq_wd_blade && eq_wd_blade2 && eq_wd_blade3 == type )
		{
#ifdef _BIRDSOUND
			ZGetSoundEngine()->PlaySoundCharacter("fx_dagger_sheath",GetPosition(),IsObserverTarget());
#else
...
void ZCharacter::OnDamagedFalling(float fDamage)
{
	...
	SetLastAttacker(m_UID);
	SetLastDamageType(ZD_FALLING);
...
void ZCharacter::InitProperties()
{
	...
	if (ZApplication::GetInstance()->GetTagScanner()->Find(ZTN_VANILLA, TRUE) == true)
	{
...
void ZCharacter::OnDamagedAPlayer(ZObject* pAttacker, MUID pVictim, MMatchWeaponType nWeaponType, RMeshPartsType nPartType, MMatchCharItemParts nItemPart, ZDAMAGETYPE nDamageType, int nMeleeType)
{
	if (pAttacker != NULL)
	{
		if (pAttacker == ZGetGame()->m_pMyCharacter && ZGetGame()->m_pMyCharacter->IsDie() == false)
		{
			ZCharacter* pVictim2 = (ZCharacter*) ZGetCharacterManager()->Find(pVictim);
			if (pVictim2 == NULL)
				return;

			if (pVictim2->IsDie() == true)
				return;
			...
			shotInfo.nVictimLowID = pVictim2->GetUID().Low;
			shotInfo.nVictimHighID = pVictim2->GetUID().High;
			shotInfo.nAttackerLowID = pAttacker->GetUID().Low;
			shotInfo.nAttackerHighID = pAttacker->GetUID().High;
			shotInfo.nWeaponType = nWeaponType;
			shotInfo.nPartType = nPartType;
			shotInfo.nItemPart = nItemPart;
			shotInfo.nDamageType = nDamageType;
			shotInfo.nMeleeType = nMeleeType;
			shotInfo.fDirx = pVictim2->GetDirection().x;
			shotInfo.fDiry = pVictim2->GetDirection().y;
			shotInfo.fDirz = pVictim2->GetDirection().z;

			memcpy(pBlobElement, &shotInfo, sizeof(MTD_ShotInfo));

			ZPOSTCMD1(MC_MATCH_NOLEAD, MCommandParameterBlob(pBlobArray, MGetBlobArraySize(pBlobArray)));
			delete pBlobArray;
		}
	}
}
