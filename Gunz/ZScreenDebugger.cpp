...
void ZScreenDebugger::DrawActorInfo(int num, MDrawContext *pDC, ZActorBase* pActor)
{
#ifndef _PUBLISH
	char buf1[256], buf2[256];

	pActor->GetDebugInfoString(buf1, buf2, num);
	PrintText(pDC, buf1);
	PrintText(pDC, buf2);
#endif
}
...
void ZScreenDebugger::DeveloperDebugInfo(MDrawContext *pDC)
{
	int sy = 150;
...
#define WIDTH 7
	...
	NEXTLINE();
	sprintf(buffer, " - Used Memory :%*ld ", WIDTH, statex.dwMemoryLoad);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Used Physical Memory :%*I64d / %*I64d", WIDTH, statex.ullTotalPhys / DIV - statex.ullAvailPhys / DIV, WIDTH, statex.ullTotalPhys / DIV);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Used Paging file :%*I64d / %*I64d", WIDTH, statex.ullTotalPageFile / DIV - statex.ullAvailPageFile / DIV, WIDTH, statex.ullTotalPageFile / DIV);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Used Virtual Memory :%*I64d / %*I64d", WIDTH, statex.ullTotalVirtual / DIV - statex.ullAvailVirtual / DIV, WIDTH, statex.ullTotalVirtual / DIV);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Camera Position( %3.1f %3.1f %3.1f )", pCamera->GetPosition().x, pCamera->GetPosition().y, pCamera->GetPosition().z);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Camera Angle : (X %3.3f | Y %3.3f | DIST %3.3f)", pCamera->m_fAngleX, pCamera->m_fAngleZ, pCamera->m_fCurrentDist);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Speed %3.1f ( %3.1f %3.1f %3.1f ),", Magnitude(rvector(pCharacter->GetVelocity().x, pCharacter->GetVelocity().y, 0)), pCharacter->GetVelocity().x, pCharacter->GetVelocity().y, pCharacter->GetVelocity().z);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Direction ( %3.1f %3.1f %3.1f )", pCharacter->m_Direction.x, pCharacter->m_Direction.y, pCharacter->m_Direction.z);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - Position ( %3.1f %3.1f %3.1f )", pCharacter->GetPosition().x, pCharacter->GetPosition().y, pCharacter->GetPosition().z);
	OUTTEXT();
	NEXTLINE();
	sprintf(buffer, " - FPS: %.f", g_fFPS);
	OUTTEXT();
	NEXTLINE();
	time_t currentTime;
	struct tm *timeinfo;
	...
	strftime(buffer, 30, " - Time: %H:%M:%S", timeinfo);
	OUTTEXT();
	NEXTLINE();
	pDC->EndFont();
}