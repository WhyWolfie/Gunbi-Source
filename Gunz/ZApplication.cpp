...
//	}
	...
	{
		mlog("Make sure you are using the right launcher\nStartup Launcher\n");
		WinExec("GunzLauncher.exe", 1);
		return false;
	}
#endif
	return false;
...
#ifndef _DEBUG
	strFileNameZItem += "";
	strFileNameZItemLocale += "";
	strFileNameZBuff += "";
	strFileNameWorlditem += "";
	strFileNameAbuse += "";
#endif
...
*/
	...
	// Actor
	if (!ZGetActorActionMgr()->ReadXml(ZApplication::GetFileSystem(), FILENAME_ACTOR_ACTION))
	{
		mlog("Error while Load %s\n", FILENAME_ACTOR_ACTION);
		return false;
	}

	// FSM Parser
	vector<ZFSM*> vecFSM;
	if (!ZGetFSMParser()->ReadXml(ZApplication::GetFileSystem(), FILENAME_AIFSM, vecFSM, ZGetActorActionMgr()))
	{
		mlog("Error while Load %s\n", FILENAME_AIFSM);
		return false;
	}
	ZGetFSMMgr()->AddFsm(&vecFSM[0], (unsigned int)vecFSM.size());

	// NPC2 MODEL
	if (ZGetNpcMeshMgr()->LoadXmlList(FILENAME_NPC2) == -1)
	{
		mlog("Error while Load %s\n", FILENAME_NPC2);
		return false;
	}
	...
	// NPC2 DESC
	if (!MGetActorDefMgr()->ReadXml(ZApplication::GetFileSystem(), FILENAME_NPC2_DESC))
	{
	...
	// BLITZ
	if (!MGetMatchBlitzKrieg()->ReadXml(FILENAME_BLITZKRIEG, ZApplication::GetFileSystem())) {
		mlog("Error while Read %s\n", FILENAME_BLITZKRIEG);
		return false;
	}
