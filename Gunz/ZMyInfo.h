...
public:
	ZMyInfo();
	virtual ~ZMyInfo();
	bool InitCharInfo(const char* szCharName, const char* szClanName, const MMatchClanGrade nClanGrade, const MMatchSex nSex, const int nHair, const int nFace, const MMatchClassJob nClassJob);
...
#ifdef _CASHCOINS
	int					GetCashCoins() { return m_nCashCoins; }
#endif
	int					GetHP();
	int					GetAP();
	...
	MMatchClassJob		GetClassJob()		{ return m_nClassJob; }
...
#ifdef _CASHCOINS
	void SetCashCoins(int nCashCoins) { m_nCashCoins = nCashCoins; }
#endif
