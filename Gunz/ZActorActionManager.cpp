...
ZActorActionManager* ZActorActionManager::GetInstance()
{
	static ZActorActionManager pStatic;
	return &pStatic;
}
...
bool ZActorActionManager::ParseXml_ActionGrenadeShot(ZActorActionGrenadeShot* pOutGrenadeShot, MXmlElement& elem)
{
	...
	if (!elem.GetAttribute(&nDelay, "delay", 0))
	{
	...
	if (!elem.GetAttribute(&nPierce, "pierce", 50))
	{
		mlog("ERROR: xml action grenade node has no damage attr.\n");
		return false;
	}
	else
	{
		if (!(0 <= nPierce && nPierce <= 100))
		{
			mlog("ERROR: xml action grenade node has invalid pierce attr.\n");
			return false;
		}
	}
	...
	if (!elem.GetAttribute(&nYaxis, "yaxis"))
	{
		mlog("ERROR: xml action grenade node has no yaxis attr.\n");
		return false;
	}

	if (!elem.GetAttribute(&nForce, "force"))
	{
	...
	pOutGrenadeShot->SetPosPartsType(posPartsType);
	pOutGrenadeShot->SetPositionMod(vDirMod);
	pOutGrenadeShot->SetDirectionMod(vDirMod);
...
bool ZActorActionManager::ParseXml_ActionEffect(ZActorActionEffect* pOutEffect, MXmlElement& elem)
{
	...
	if (1 != sscanf(strScale.c_str(), "%f", &vScale.x))
	{
	...
	if (elem.GetAttribute(&strPosParts, "posparts"))
	{
		...
		else if (0 == stricmp(strPosParts.c_str(), ""))			posPartsType = eq_parts_pos_info_end; 
		else
		{
			mlog("ERROR: xml action range shot node has invalid posparts attr.\n");
			return false;
