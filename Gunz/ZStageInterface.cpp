...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_DROPGUNGAME) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_CLASSICMODE) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_DUEL))							// 듀얼모드 이면...
	{
		...
		(pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM2) ||			// 무한데스매치 팀전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_CLASSICMODE_TEAM) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_TRAINING_TEAM) ||
		 (pSetting->nGameType == MMATCH_GAMETYPE_CTF))					// 암살전 이면...
	{
		...
		bQuestUI = false;
			...
			pSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)					// 챌린지퀘스트 모드//todok 따로 해야할까?
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
			pAniMapImg->SetCurrentFrame( 0);
...
/***********************************************************************
  ...
  desc : 맵 선택 콤보박스에 현재 선택된 맵 이름을 세팅한다.
  arg  : szMapName = 맵 이름
  ret  : none
************************************************************************/
