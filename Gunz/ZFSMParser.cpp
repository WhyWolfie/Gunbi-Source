...
bool ZFSMParser::ParseFunction(ZFSMFunctionCall* out_pFuncCall, const char* sz)
{
	...
	else if (!stricmp(szFuncName, "faceToLastestAttacker")) { funcId = ZFSMFunctionServer::FSMFUNC_FACE_TO_LASTEST_ATTACKER; if (numArg != 0) return false; }
	else if (!stricmp(szFuncName, "runWaypointsAlongRoute")) { funcId = ZFSMFunctionServer::FSMFUNC_RUN_WAY_POINTS_ALONG_ROUTE; if (numArg != 0) return false; }
	if (funcId == ZFSMFunctionServer::FSMFUNC_INVALID)
	{
		mlog("Unknown FSM function name : \'%s\'\n", szFuncName);

#ifdef _FSM_ALLOWUNKNOWN
		funcId = ZFSMFunctionServer::FSMFUNC_ROTATE_TO_TARGET;
#else
		return false;
#endif
	}
...
bool ZFSMParser::ParseCondition( ZFSMCondition* out_pCond, const char* sz )
{
	...
	else if (!stricmp(szCondName, "SummonLess"))				{ condId = FSMCOND_SUMMONLESS;					if (numArg != 1) return false; }

	if (condId == FSMCOND_INVALID)
	{
		mlog("Unknown FSM condition name : \'%s\'\n", szCondName);
#ifdef _FSM_ALLOWUNKNOWN
		condId = FSMCOND_DEFAULT;
#else
		return false;
#endif
	}
...
ZFSMParser* ZFSMParser::GetInstance()
{
	static ZFSMParser pStatic;
	return &pStatic;
}