...
void ZShopEquipInterface::OnBuyButton(void)
{
	...
	ZShopEquipListItem* pListItem;

	if (ZGetShop()->GetShopMode() == SHOP_CASH)
	{
		pListItem = (ZShopEquipListItem*) GetListCurSelItem("CashEquipmentList");
	}
	else
	{
		pListItem = (ZShopEquipListItem*) GetListCurSelItem("AllEquipmentList");
	}
...
{
	// Select Shop Type
	if (nTabIndex == 0)			ZGetShop()->SetShopMode(SHOP_NORMAL);
	else if (nTabIndex == 2)	ZGetShop()->SetShopMode(SHOP_CASH);
...
void ZShopEquipInterface::SelectEquipmentTab(int nTabIndex)
{
	...
	pWidget = pResource->FindWidget("CraftingItemList");
	if (pWidget != NULL) pWidget->Show(nTabIndex == 0 ? false : true);
	...
	pButton = (MButton*)pResource->FindWidget("Equipment_CraftingTab");
	if (pButton)
		pButton->Show(nTabIndex == 1 ? false : true);
	...
	pLabel = (MLabel*)pResource->FindWidget("Equipment_FrameTabLabel3");
	if (pLabel)
		pLabel->Show(nTabIndex == 2 ? true : false);
	...
	if ( pPicture)
		pPicture->Show( nTabIndex==1 ? true : false);
	pPicture = (MPicture*)pResource->FindWidget("Equip_ListLabel3");
	if (pPicture)
		pPicture->Show(nTabIndex == 2 ? true : false);
...
void ZShopEquipInterface::DrawCharInfoText(char* szShopOrEquip, int nReqLevel, int nNewWT, int nNewMaxWT, int nNewHP, int nNewAP, int nReqBounty, int nReqCash)
{
...
#else
	MSHOPMODE nShopMode = ZGetShop()->GetShopMode();
	bool bIsCash = nShopMode == SHOP_CASH;
	bool bIsBounty = !bIsCash;

	if (strcmp(szShopOrEquip, "Equip") == 0) 
	{
		bIsCash = false;
		bIsBounty = false;
	}
	...
	sprintf(sz1, "^9%s : ", ZMsg(MSG_CHARINFO_BOUNTY));
	szColor = ((nReqBounty > ZGetMyInfo()->GetBP()) && bIsBounty) ? szRed : szGray;
	sprintf(sz2, "%s%d", szColor, ZGetMyInfo()->GetBP());
	strcat(sz1, sz2);
	pTextArea[2]->AddText(sz1);

	sprintf(sz1, "^9%s : ", "P-Cash");
	szColor = ((nReqBounty > ZGetMyInfo()->GetCashCoins()) && bIsCash) ? szRed : szGray;
	sprintf(sz2, "%s%d", szColor, ZGetMyInfo()->GetCashCoins());
	strcat(sz1, sz2);
	pTextArea[2]->AddText(sz1);
