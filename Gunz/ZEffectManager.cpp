...
bool ZEffectManager::Create(void)
{
	...
	m_pRedCTFEffect = m_pEffectMeshMgr->Get("ef_wctf_r");
	m_pBlueCTFEffect = m_pEffectMeshMgr->Get("ef_wctf_b");
...
void ZEffectManager::AddRedCTFIcon(ZObject* pObj)
{
	ZEffect* pNew = NULL;

		pNew = new ZEffectBerserkerIconLoop(m_pRedCTFEffect, pObj);
		((ZEffectIconLoop*)pNew)->SetAlignType(1);
		((ZEffectIconLoop*)pNew)->m_type = eq_parts_pos_info_Spine2;
		Add(pNew);
	}

void ZEffectManager::AddBlueCTFIcon(ZObject* pObj)
{
	ZEffect* pNew = NULL;

			pNew = new ZEffectBerserkerIconLoop(m_pBlueCTFEffect, pObj);
			((ZEffectIconLoop*)pNew)->SetAlignType(1);
			((ZEffectIconLoop*)pNew)->m_type = eq_parts_pos_info_Spine2;
			Add(pNew);
}
...
void ZEffectManager::AddWithScale(const char* szName, const rvector& pos, const rvector& dir, const MUID& uidOwner, const float& fDelay, const rvector& scale)
{
	RMesh *pMesh = m_pEffectMeshMgr->Get(szName);

	if (!pMesh) {
		return;
	}

	ZEffect* pNew = new ZEffectSlash(pMesh, (rvector&)pos, (rvector&)dir);
	if (!pNew) {
		return;
	}
