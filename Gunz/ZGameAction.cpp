		...
		case ZC_SKILL_UPPERCUT:
			{
				if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE_TEAM)			
					break;
				OnPeerSkill_Uppercut(pOwnerCharacter);
			}
			break;
		case ZC_SKILL_SPLASHSHOT:
			{
				if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE_TEAM)		
					break;
	
				OnPeerSkill_LastShot(fTime, pOwnerCharacter);
			}
			break;
		case ZC_SKILL_DASH:
			{ 
				...
				OnPeerSkill_Dash(pOwnerCharacter);
			}
			break;
	}
		...
		itor != ZGetObjectManager()->end(); ++itor)
	{
		...
		if (fDist < 200.0f) {
			...
			if ((pTar) && (pTar != pOwnerCharacter))
			{
				...
				if(ZGetGame()->IsWallBlocked(pOwnerCharacter,pTar)==true) //중간에 벽이 막고 있는가?
					bCheck = false;
