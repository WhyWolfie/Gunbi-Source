	...
	if (ZIsActionKeyPressed(ZACTION_USE_WEAPON2))
	{
		...
		if (pSelectedItem)
		{
			if(pSelectedItem->GetDesc()) {
				...
				case MWT_SNIFER:
					{
						if(clock() < SniferTimer) {break;}
						SniferTimer = clock () + 0.35 * CLOCKS_PER_SEC ;
						OnGadget_Snifer();
					}
					break;
				}
...
void ZMyCharacter::ProcessGuard()
{
	...
	if(type!=MWT_KATANA && type!=MWT_DOUBLE_KATANA && type != MWT_KATANA1 && type != MWT_KATANA2 && type != MWT_KATANA3 && type != MWT_KATANA4
		&& type != MWT_AXE && type != MWT_BASEBALL && type != MWT_FIGHTER && type != MWT_DOUBLE_KATANA2 && type != MWT_DOUBLE_KATANA3)
	{
		CHECK_RETURN_CALLSTACK(ProcessGuard);
		return; 
	}
...
//	if(GetItems()->GetSelectedWeapon()->IsEmpty() == true) return;		// 총알부족
	...
	*/
	...
	switch( m_pVMesh->GetSelectWeaponMotionType() ) {
		...
		case eq_wd_sword2 : 
		case eq_wd_blade2 : 
		case eq_wd_blade3 :		OnShotMelee();	break;
...
//		if(m_bShot) 
		{
				...
				itor != ZGetCharacterManager()->end(); ++itor)
			{
				...
				if (fDist < 200.0f) {
					...
					if(ZGetGame()->IsWallBlocked(this,pTar)==true) //중간에 벽이 막고 있는가?
						bCheck = false;
...
float ZMyCharacter::GetGravityConst()	// 중력의 영향을 얼만큼 받는지..
{
	...
	if(zStatus.m_bSlash)
	{
		...
		if(pDesc->m_nWeaponType.Ref()==MWT_DOUBLE_KATANA || MWT_DOUBLE_KATANA2 || MWT_DOUBLE_KATANA3) {
			AniFrameInfo* pAniLow = m_pVMesh->GetFrameInfo(ani_mode_lower);
...
void ZMyCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if (ZGetGame()->m_pMyCharacter->GetClassJob() == MMCJ_DEFFENDER || ZGetGame()->m_pMyCharacter->GetClassJob() == MMCJ_SOLDIER)
	{

		if (weaponType == MWT_MACHINEGUN || weaponType == MWT_RIFLE)
		{
			if (pMItemDesc->m_nLimitShield.Ref() <= 100)		//정상 유저들..
				fDamage = pMItemDesc->m_nLimitShield.Ref() * 1;
	...
	{
		if (weaponType == MWT_SNIFER)
			ZPostVampireHealt(pAttacker->GetUID());
	}
#endif

	ZGetScreenEffectManager()->AddAlert(GetPosition(),m_Direction, srcPos);

	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_SURVIVAL && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		if (!pAttacker->IsNPC())
		{
			...
			if (pCharacter != ZGetGame()->m_pMyCharacter)
			{
				if (fDamage != 0)
				{
					ZPostDamageCounter(fDamage, pAttacker->GetUID());
				}
			}
		}
	}
