...
string GetItemSpecString(MMatchItemDesc* pItemDesc)
{
	...
	else
	{
		...
		if (pItemDesc->m_nSF.Ref()) {
			sprintf(temp, "%s +%d\n", "Normal Resistance", pItemDesc->m_nSF.Ref());
			str += temp;
		}
		if (pItemDesc->m_nFR.Ref()) {
			sprintf(temp, "%s +%d\n", ZMsg(MSG_WORD_FR), pItemDesc->m_nFR.Ref());
			str += temp;
		}
		if (pItemDesc->m_nCR.Ref()) {
			sprintf(temp, "%s +%d\n", ZMsg(MSG_WORD_CR), pItemDesc->m_nCR.Ref());
			str += temp;
		}
		if (pItemDesc->m_nPR.Ref()) {
			sprintf(temp, "%s +%d\n", ZMsg(MSG_WORD_PR), pItemDesc->m_nPR.Ref());
			str += temp;
		}
		if (pItemDesc->m_nLR.Ref()) {
			sprintf(temp, "%s +%d\n", ZMsg(MSG_WORD_LR), pItemDesc->m_nLR.Ref());
			str += temp;
		}
		if (pItemDesc->m_nControllability.Ref()) {
			...
			case MWT_SNIFER:
			case MWT_BOWS:
				sprintf(temp, "%s : %d(/sen)\n", "Control", pItemDesc->m_nControllability.Ref());
				str += temp;
				break;
			}
		}
			...
			if (pItemDesc->m_nLimitShield.Ref()) 
			{
				if (ZGetMyInfo()->GetClassJob() == MMCJ_SOLDIER || ZGetMyInfo()->GetClassJob() == MMCJ_DEFFENDER)
				{
					switch (pItemDesc->m_nWeaponType.Ref()) {
					case MWT_RIFLE:
					case MWT_MACHINEGUN:
						sprintf(temp, "%s : %d(Meningkat)\n", "Pertahanan", pItemDesc->m_nLimitShield.Ref());
						str += temp;
						break;
					}
				}
...
void ZShopEquipItemHandle_PurchaseMatch::Buy()
{
	...
	if (bCash)	// 캐쉬 아이템
	{
#if !defined(_PUBLISH) || defined(_EXTRASHOPS)
