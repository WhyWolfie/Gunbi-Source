...
bool ZFSMCondition::Check(IFSMConditionSource* pSrc)
{
	...
	case FSMCOND_ANGLE_TARGET_HEIGHT:
		{
			float fAngleLow = m_nArg[0];
			float fAngleHigh = m_nArg[1];
			return pSrc->Cond_AngleTargetHeight(fAngleLow, fAngleHigh);
		}
	case FSMCOND_TIME_ELAPSED_SINCE_ENTERED:
		{
			DWORD dwElapsed = ZTimeGetTime() - pSrc->Cond_GetStateEnteredTime();
