...
RRESULT OnCreate(void *pParam)
{
	...
	sprintf(cstrReleaseDate, "v1.5.1.0.0");
...
//	if( !g_pDefFont->Create("Default", RGetDevice(), "FONTb11b", 14, 1.0f, true, false) )
	...
	if( ZGetInitialLoading()->IsUseEnable() )
	{
		ZGetInitialLoading()->AddBitmap( 0, "Interface/Default/LOADING/loading_bg.jpg" );
		ZGetInitialLoading()->AddBitmapBar( "Interface/Default/LOADING/loading_fg.jpg" );
		ZGetInitialLoading()->SetPercentage( 0.0f );
		ZGetInitialLoading()->Draw( MODE_FADEIN, 0 , true );
	}
#endif
...
void GuardPtrs()
{
	float(__thiscall ZCharacter::* pGetAP)() = &ZCharacter::GetAP;
	void(__thiscall ZCharacter::* pGetHP)(float) = &ZCharacter::SetAP;

	void* pPtrGetAP = (void*&)pGetAP;
	void* pPtrGetHP = (void*&)pGetHP;
}
...
//		AfxMessageBox(_T("_AhnHS_SaveFuncAddress Failed!"));
	...
	mlog("GUNZ %s launched. build (%s %s) \n", STRFILEVER, __DATE__, __TIME__);
	char szDateRun[128]="";
	char szTimeRun[128]="";
	_strdate( szDateRun );
	_strtime( szTimeRun );
