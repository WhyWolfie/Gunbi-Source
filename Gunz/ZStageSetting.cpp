...
#define BUILD_STAGESETTING_ITEM(_WidgetItemName, _NodeVariable, _ItemList, _nItemCount)		\
	...
	// 제한레벨
	BUILD_STAGESETTING_ITEM("StageLevelLimit", pOutNode->nLimitLevel, StageSetting_LimitLevel, STAGESETTING_LIMITLEVEL_MAX);

	// 팀킬여부
	BUILD_STAGESETTING_ITEM("StageTeamKill", pOutNode->bTeamKillEnabled, StageSetting_TeamKill, STAGESETTING_TEAMKILL_MAX);

	// 난입여부
	BUILD_STAGESETTING_ITEM("StageIntrude", pOutNode->bForcedEntryEnabled, StageSetting_ForcedEntry, STAGESETTING_FORCEDENTRY_MAX);

	// 팀 밸런스
	BUILD_STAGESETTING_ITEM("StageTeamBalancing", pOutNode->bAutoTeamBalancing, StageSetting_TeamBalancing, STAGESETTING_FORCEDENTRY_MAX);
...
#define INITSTAGESETTING_LISTITEM( _WidgetItemName, _ItemList, _nItemDefaultIndex)	\
	...
	if (pCBType && (pCBType->GetCount() > 0))
	{
		...
		if (strcmp(pCBType->GetString(pCBType->GetSelIndex()), ZMsg(MSG_MT_DEATHMATCH_SOLO)) == 0)
			...
			nGameType = 12;
		else if (strcmp(pCBType->GetString(pCBType->GetSelIndex()), "Drop Gun Game") == 0)
			nGameType = 13;
