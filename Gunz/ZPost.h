...
{ \
	if (Damage > 0) \
	{ \
		ZPOSTCMD2(MC_PEER_DAMAGECOUNTER, MCommandParameterInt(Damage), MCommandParameterUID(AttackerUID)); \
	} \
}

#define ZPostKillStreak(uid1, uid2, szName, nKillStreakCount, nOldKillStreakCount, szName2) \
{ \
	ZPOSTCMD4(MC_PEER_KILLSTREAK, MCmdParamStr(szName), MCmdParamInt(nKillStreakCount), MCmdParamInt(nOldKillStreakCount), MCmdParamStr(szName2)); \
	ZPOSTCMD6(MC_PEER_KILLSTREAK, MCmdParamUID(uid1),MCmdParamUID(uid2), MCmdParamStr(szName), MCmdParamInt(nKillStreakCount), MCmdParamInt(nOldKillStreakCount), MCmdParamStr(szName2)); \
}
#define ZPostBankai(b) \
{	\
	ZPOSTCMD1(MC_PEER_BANKAI, MCmdParamBool(b));	\
}

#define ZPostPayback(szName, szName2) \
{ \
	ZPOSTCMD2(MC_PEER_PAYBACK, MCmdParamStr(szName), MCmdParamStr(szName2)); \
}

#define ZPostVampireHealt(AttackerUID) \
{ \
	ZPOSTCMD1(MC_PEER_VAMPIREHEALT, MCommandParameterUID(AttackerUID)); \
} 

#define ZPostHPAP(AttackerUID, nHP,nAP) \
{ \
	ZPOSTCMD3(MC_PEER_HPAP, MCmdParamUID(AttackerUID), MCmdParamInt(nHP), MCmdParamInt(nAP)); \
}

#define ZPostVampireDeath(AttackerUID, nHP,nAP) \
{ \
	ZPOSTCMD3(MC_PEER_VAMPIRE_DEATH, MCmdParamUID(AttackerUID), MCmdParamInt(nHP), MCmdParamInt(nAP)); \
}
...
inline void ZPostWarRequestJoinGame() {
	ZPOSTCMD0(MC_WAR_GAME_REQUEST_JOINGAME);
}
...
#ifdef _CLOSECHANNEL
inline void ZPostAdminOpenCloseChannel(const MUID& uidChannel, bool bIsOpen) 
{
	ZPOSTCMD2(MC_ADMIN_OPENCLOSE_CHANNEL, MCmdParamUID(uidChannel), MCmdParamBool(bIsOpen));
}
#endif
...
#define ZPostRequestCaptureFlag(nItemUID) \
{ \
	MCommand *pNewCmd = ZNewCmd(MC_MATCH_REQUEST_FLAG_CAP); \
	pNewCmd->AddParameter(new MCommandParameterInt(nItemUID)); \
	ZPostCommand(pNewCmd); \
}
