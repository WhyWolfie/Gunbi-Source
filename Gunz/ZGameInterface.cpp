...
string ClassJobToString2(MMatchClassJob i)
{
	if (i == MMCJ_ASSASIN)
		return "Assassin";
	else if (i == MMCJ_COWBOY)
		return "Cowboy";
	else if (i == MMCJ_DEFFENDER)
		return "Deffender";
	else if (i == MMCJ_SOLDIER)
		return "Soldier";
	else
		return "Unknown";
}
...
bool ZGameInterface::InitInterfaceListener()
{
	...
	*/
	...
	SetListenerWidget("WarGame", ZGetWarGameButtonListener());
	...
	SetListenerWidget("ChannelList_War", ZGetChannelList());
	SetListenerWidget("MyClanChannel", ZGetMyClanChannel());
	SetListenerWidget("MyClanChannel2", ZGetMyClanChannel());
	...
	*/
	...
	SetListenerWidget("CashEquipmentList", ZGetShopPurchaseItemListBoxListener());
...
bool ZGameInterface::OnGameCreate(void)
{
	...
	case (7) :
		...
		break;
	case (8) :
		strcpy(szFileName, "Interface/Default/LOADING/loading9.jpg");
		break;
	case (9) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_10.jpg");
		break;
	case (10) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_11.jpg");
		break;
	case (11) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_12.jpg");
		break;
	case (12) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_13.jpg");
		break;
	case (13) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_14.jpg");
		break;
	case (14) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_15.jpg");
		break;
	case (15) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_16.jpg");
		break;
	case (16) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_17.jpg");
		break;
	case (17) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_18.jpg");
		break;
	case (18) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_19.jpg");
		break;
	case (19) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_20.jpg");
		break;
	case (20) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_21.jpg");
		break;
	case (21) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_22.jpg");
		break;
	case (22) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_23.jpg");
		break;
	case (23) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_24.jpg");
		break;
	case (24) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_25.jpg");
		break;
	case (25) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_26.jpg");
		break;
	case (26) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_27.jpg");
		break;
	case (27) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_28.jpg");
		break;
	case (28) :
		strcpy(szFileName, "Interface/Default/LOADING/loading_29.jpg");
		break;
	}
...
void ZGameInterface::InitLobbyUIByChannelType()
{
	...
	bool BWarUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_WAR);

	if (bClanBattleUI)
	{
		ZGetGameInterface()->InitClanLobbyUI(true);
		ZGetGameInterface()->InitDuelTournamentLobbyUI(false);
		ZGetGameInterface()->InitWarLobbyUI(false);
	}
	else if (bDuelTournamentUI)
	{
		...
		ZGetGameInterface()->InitClanLobbyUI(false);
		ZGetGameInterface()->InitDuelTournamentLobbyUI(false);
		ZGetGameInterface()->InitWarLobbyUI(true);
	}
	else
	{
		ZGetGameInterface()->InitClanLobbyUI(false);
...
bool ZGameInterface::OnCreate(ZLoadingProgress *pLoadingProgress)
{
	...
	*/
	...
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_QUEST_CHALLENGE, "Challenge Quest");	//todok ZMsg로 변경할 것
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_DROPGUNGAME, "Drop Gun Game");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_CTF, "Capture The Flag");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_SNIPERMODE, "Gunner Battle");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_CLASSICMODE, "Classic");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_CLASSICMODE_TEAM, "Classic Team");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_TRAINING_TEAM, "Training Team");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_BLITZKRIEG, "BlitzKrieg");
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_WAR, "War Mode");
...
void ZGameInterface::OnDrawStateLobbyNStage(MDrawContext* pDC)
{
	...
	if (GetState() == GUNZ_LOBBY)
	{
		...
		if (pLabel) pLabel->SetText(buf);
		//ClassJob
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecClassJob");
		sprintf(buf, "%s : %s", "Class", ClassJobToString2(ZGetMyInfo()->GetClassJob()).c_str());
		if (pLabel) pLabel->SetText(buf);

		// Cash Coins
#ifdef _CASHCOINS
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecCashCoins");
		sprintf(buf, "%s : %d", "P-Cash", ZGetMyInfo()->GetCashCoins());
		if (pLabel) pLabel->SetText(buf);
#endif
	...
	else if (GetState() == GUNZ_STAGE)
	{
		...
		if (pListBox)
		{
			...
			// Classjob
			pLabel = (MLabel*)pRes->FindWidget("Stage_PlayerClass");
			if (pLabel)
			{
				if (bShowMe)
					sprintf(buf, "%s : %s", "Class", ClassJobToString2(ZGetMyInfo()->GetClassJob()).c_str());
...
void ZGameInterface::ChangeWeapon(ZChangeWeaponType nType)
{
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE_TEAM)
	{
		switch (nType)
		{
		case(1) :
...
bool GetItemDescStr(string& str, DWORD nItemID) {
	...
	if (pItemDesc->m_nWeight.Ref()){
		if (bAdd) str += " / ";
		sprintf(temp, "무게:%d", pItemDesc->m_nWeight.Ref());
	...
	if (pItemDesc->m_nMaxBullet.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "최대탄수 : %d", pItemDesc->m_nMaxBullet.Ref());
	...
	if (pItemDesc->m_nMagazine.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "탄창 : %d", pItemDesc->m_nMagazine.Ref());
	...
	if (pItemDesc->m_nDamage.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "공격력 : %d", pItemDesc->m_nDamage.Ref());
	...
	if (pItemDesc->m_nDelay.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "딜레이 : %d", pItemDesc->m_nDelay.Ref());
	...
	if (pItemDesc->m_nReloadTime.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "장전시간 : %d", pItemDesc->m_nReloadTime.Ref());
	...
	if (pItemDesc->m_nMaxWT.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "+최대무게 : %d", pItemDesc->m_nMaxWT.Ref());
...
void ZGameInterface::InitWarLobbyUI(bool bEnabled)
{
	MWidget *pWidget;

	if (bEnabled) {
		pWidget = m_IDLResource.FindWidget("Lobby_RoomListBG");
		if (pWidget) pWidget->Show(!bEnabled);
	...
	pWidget = m_IDLResource.FindWidget("WarGame");
	if (pWidget) pWidget->Show(bEnabled);
}
...
#elif defined(LOCALE_KOREA) || defined(_DEBUG)
			//		-------------------------
			//		|			|	클랜전	|
			//		|	일반서버|	nRow=1	|
			//		|	nRow=0 	|	nCol=0	|
