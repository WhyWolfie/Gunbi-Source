...
public:
	ZLobbyPlayerListItem(const MUID& puid, MBitmap* pBitmap, unsigned int nClanID, const char* szLevel, const char* szName, 
		const char *szClanName, ePlayerState nLobbyPlayerState,MMatchUserGradeID Grade, MBitmap* pBmpDTGradeIcon, int nKills, int nDeaths)
	{
		m_pBitmap = pBitmap;
...
//	MBitmap* GetBitmapIn() { return m_pBitmapIn; }
	...
	void AddPlayer(MUID& puid, ePlayerState state, int  nLevel,char* szName, char* szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade , int nKills, int nDeaths);

	// mode PLAYERLISTMODE_STAGE
	void AddPlayer(MUID& puid, MMatchObjectStageState state, int nLevel, char* szName, char* szClanName, unsigned int nClanID, bool isMaster, MMatchTeam nTeam, int duelTournamentGrade, int nKills, int nDeaths);
