...
{
	if (!ZGetGame()->GetMatch()->IsTeamPlay()) return; // enable radar only in team play
	if (ZGetGame()->GetMatch()->IsQuestDrived()) return;
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL) return;
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUELTOURNAMENT) return;
