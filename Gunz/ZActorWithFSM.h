...
private:
	...
	set<const ZActorActionShotCommon*> m_setProcessedShotInAction;	
	...
	set<const ZActorActionEffect*> m_setProcessedEffectInAction;	
	...
	set<const ZActorActionSound*> m_setProcessedSoundInAction;	
...
public:
	...
	virtual MMatchTeam GetTeamID() { return m_nTeamID.Ref(); }			// NPC는 무조건 BLUE팀
...
public:
	...
	virtual bool Cond_AngleTargetHeight(float fActorPosLow, float fActorPosHigh);
...
public:
	...
	virtual void Func_FindTargetInDist(int nArg1);
	virtual void Func_FaceToLastestAttacker();
	virtual void Func_RunWaypointsAlongRoute();
...
public:
	...
	bool IsAir() {
		return (GetDistToFloor() > 0.1f || GetVelocity().z > 0.1f);
	}

	int HasSpeedMod() { 
		return m_nNewSpeedMod; 
	}

	void SetSpeedMod(int i) {
		m_nNewSpeedMod = i; 
	}
