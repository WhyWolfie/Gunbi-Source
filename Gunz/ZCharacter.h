...
//	void OnHeal(ZCharacter* pAttacter,int type,int heal);
	...
	MMatchClassJob GetClassJob() { return m_MInitialInfo.Ref().nClassJob; }
...
//	void SetSafeFall(int nSafeFall) { m_Property.nSafeFall = nSafeFall; }
	...
	int GetDeaths() { return GetStatus().Ref().nDeaths; }
...
//	float GetMeleeWeaponRange();
	...
	void OnDamagedFalling(float fDamage);
...
//	virtual void OnDamage(int damage, float fRatio = 1.0f);
	...
	virtual void OnDamagedAPlayer(ZObject* pAttacker, MUID pVictim, MMatchWeaponType nWeaponType, RMeshPartsType nPartType, MMatchCharItemParts nItemPart, ZDAMAGETYPE nDamageType, int nMeleeType = -1);
...
protected:
	MUID PaybackUID;
public:
	void SetPayBackChar(MUID m) { PaybackUID = m; }
	MUID GetPayBackChar() { return PaybackUID; }
};
