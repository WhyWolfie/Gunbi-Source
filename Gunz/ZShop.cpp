...
#ifdef _EXTRASHOPS
				if (m_nShopMode != SHOP_NORMAL) continue;
#endif
		...
		for (int i = 0; i < GetItemCount(); i++)
		{
			...
			if (MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(m_vShopItem[i]->nItemID))
			{
...
#ifdef _EXTRASHOPS
				if (pDesc->IsCashItem() && m_nShopMode != SHOP_CASH) continue;
				if (!pDesc->IsCashItem() && m_nShopMode != SHOP_NORMAL) continue;
#endif

#ifdef _CLASSJOB_SHOP_FILTER_CLIENT
				...
				if (pItem > 0 && pItem < 18)
				{
					...
					{
						if (pItem != MWT_DAGGER &&
							pItem != MWT_DUAL_DAGGER &&
							pItem != MWT_KATANA &&
							pItem != MWT_GREAT_SWORD &&
							pItem != MWT_DOUBLE_KATANA &&
							pItem != MWT_PISTOL &&
							pItem != MWT_PISTOLx2 &&
							pItem != MWT_REVOLVER &&
							pItem != MWT_REVOLVERx2 &&
							pItem != MWT_SMG &&
							pItem != MWT_SMGx2 &&
							pItem != MWT_ROCKET &&
							pItem != MWT_SNIFER) {
					...
					else if (ZGetMyInfo()->GetClassJob() == MMCJ_DEFFENDER)
					{
						if (pItem != MWT_DAGGER &&
							pItem != MWT_DUAL_DAGGER &&
							pItem != MWT_KATANA &&
							...
							pItem != MWT_SMGx2 &&
							pItem != MWT_ROCKET &&
							pItem != MWT_MACHINEGUN) {
							continue;
						}
					}
					else if (ZGetMyInfo()->GetClassJob() == MMCJ_COWBOY)
					{
						if (pItem != MWT_DAGGER &&
							pItem != MWT_DUAL_DAGGER &&
							pItem != MWT_KATANA &&
							...
							pItem != MWT_ROCKET &&
							pItem != MWT_SHOTGUN &&
							pItem != MWT_SAWED_SHOTGUN) {
							continue;
						}
					}
					else if (ZGetMyInfo()->GetClassJob() == MMCJ_SOLDIER)
					{
						if (pItem != MWT_DAGGER &&
							pItem != MWT_DUAL_DAGGER &&
							pItem != MWT_KATANA &&
