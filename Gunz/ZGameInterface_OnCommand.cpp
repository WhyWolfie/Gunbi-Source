	...
	case MC_MATCH_RESPONSE_SELECT_CHAR:
		{
			...
			if (nResult == MOK)
			{
				...
				ZGetMyInfo()->InitCharInfo(pCharInfo->szName, pCharInfo->szClanName, pCharInfo->nClanGrade, (MMatchSex)pCharInfo->nSex, (int)pCharInfo->nHair, (int)pCharInfo->nFace, (MMatchClassJob)pCharInfo->nClassJob);				
				...
				char WelcomeText[255];
				sprintf(WelcomeText, "Welcome ^2%s^9 to GSO.(http://pandorags.com/)", pCharInfo->szName);
				ZApplication::GetGameInterface()->ShowMessage(WelcomeText);
...
#ifdef _CASHCOINS
			pCommand->GetParameter(&nCashCoins, paramIndex++, MPT_INT);
			ZGetMyInfo()->SetCashCoins(nCashCoins);
#endif
