...
#define ZTOK_SPAWN_BLITZ_RADAR		"spawn_blitz_radar"
#define ZTOK_SPAWN_BLITZ_BARRICADE	"spawn_blitz_barricade"
#define ZTOK_SPAWN_BLITZ_HONORITEM  "spawn_blitz_honoritem"
#define ZTOK_SPAWN_BLITZ_GUARDIAN	"spawn_blitz_guardian"
...
bool ZMapSpawnManager::Add(ZMapSpawnData* pMapSpawnData)
{
	...
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_BLITZ_RADAR, strlen(ZTOK_SPAWN_BLITZ_RADAR)))
	{
		pMapSpawnData->m_nType = ZMST_NPC_BLITZ_RADAR;
		m_SpawnArray[ZMST_NPC_BLITZ_RADAR].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_BLITZ_BARRICADE, strlen(ZTOK_SPAWN_BLITZ_BARRICADE)))
	{
		pMapSpawnData->m_nType = ZMST_NPC_BLITZ_BARRICADE;
		m_SpawnArray[ZMST_NPC_BLITZ_BARRICADE].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_BLITZ_HONORITEM, strlen(ZTOK_SPAWN_BLITZ_HONORITEM)))
	{
		pMapSpawnData->m_nType = ZMST_NPC_BLITZ_HONORITEM;
		m_SpawnArray[ZMST_NPC_BLITZ_HONORITEM].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_BLITZ_GUARDIAN, strlen(ZTOK_SPAWN_BLITZ_GUARDIAN)))
	{
		pMapSpawnData->m_nType = ZMST_NPC_BLITZ_GUARDIAN;
		m_SpawnArray[ZMST_NPC_BLITZ_GUARDIAN].push_back(pMapSpawnData);
	}
	return true;
}
