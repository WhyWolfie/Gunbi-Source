...
#ifdef _TDD
// 단위테스트를 돌리는 빌드에서 실행 시점에 따라 실제로 커맨드를 보낼지 말지를 설정할 수 있는 플래그

bool g_bTddNeverPostCmd = false;

bool ZTddNeverPostCmd()
{
	return g_bTddNeverPostCmd;
}
#endif
									  ...
									  const MUID& uidPlayer, bool bEnabledSurvivalMode, bool bEnabledDuelTournament, unsigned char* pbyGuidReqMsg)
{
	int nRet = MMatchClient::OnResponseMatchLogin(uidServer, nResult, szServerName, nServerMode,
												  szAccountID, nUGradeID, nPGradeID, 
												  #ifdef _CASHCOINS
												  nCashCoins,
												  #endif
												  uidPlayer, bEnabledSurvivalMode, bEnabledDuelTournament, pbyGuidReqMsg);
	...
	if(pList)
	{
		if (nType == MATCHCACHEMODE_UPDATE) {
			...
			for(int i=0; i<nCount; i++){
				...
				{	
					pList->AddPlayer(pCache->GetUID(),MOSS_NONREADY,pCache->GetLevel(),pCache->GetName(),pCache->GetClanName(),pCache->GetCLID(),false,MMT_ALL, pCache->GetDTGrade(), pCache->GetKillCount(), pCache->GetDeathCount());
...
void ZGameClient::OnStageJoin(const MUID& uidChar, const MUID& uidStage, unsigned int nRoomNo, char* szStageName, bool isWar)
{
...
//	gid = MMUG_DEVELOPER;
	...
	{
		wsprintf(szText, "^4%s : %s", name.c_str(), szChat);
	...
	for(int i=0; i<nCount; i++) 
	{
		...
		if( pNode ) 
		{
			...
			if ((pNode->nPlayerFlags & MTD_PlayerFlags_AdminHide) == true) {
				...
				pPlayerListBox->AddPlayer(pNode->uidPlayer, state, pNode->nLevel, pNode->szName, pNode->szClanName, pNode->nCLID, (MMatchUserGradeID)pNode->nGrade, pNode->nDTLastWeekGrade, pNode->nKills, pNode->nDeaths);
			}
		}
	}
...
#else
			ZTransMsg(szMsg, MSG_GAME_LEVEL_UP, 1, pCharacter->GetUserAndClanName());
#endif
			ZChatOutput(MCOLOR(ZCOLOR_GAME_INFO), szMsg);
		}
	}
}
...
int ZGameClient::ValidateRequestDeleteChar()
{
	...
	// 캐쉬아이템이 있으면 삭제할 수 없다
	//NICK: Force char delete even if it has cash
