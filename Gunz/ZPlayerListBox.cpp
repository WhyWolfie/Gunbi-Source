...
void ZPlayerListBox::AddPlayer(MUID& puid, ePlayerState state, int  nLevel, char* szName, char *szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade, int nKills, int nDeaths)
{
...
//	if(GetUserInfoUID(puid,_color,sp_name,gid)) {
...
#ifdef _NEW_DT_ICON
	GetGradeKillIcon(szDTGradeIconFileName, nKills, nDeaths);
#else
	...
	ZLobbyPlayerListItem* pItem = new ZLobbyPlayerListItem(puid, MBitmapManager::Get(szFileName), nClanID, szLevel, szRefName, szClanName, state, nGrade, pBmpDTGradeIcon, nKills,nDeaths );
...
void ZPlayerListBox::AddPlayer(MUID& puid, MMatchObjectStageState state, int nLevel, char* szName, char* szClanName, unsigned int nClanID, bool isMaster, MMatchTeam nTeam, int duelTournamentGrade, int nKills, int nDeaths)
{
	if ( (int)strlen( szName) == 0)
		return;
