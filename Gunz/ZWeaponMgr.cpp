...
void ZWeaponMgr::AddNewQuestTrap(const ZActorActionGrenadeShot* pGrenadeShot, rvector &pos, rvector &velocity, ZObject* pC)
{
	if (pGrenadeShot == nullptr) {
		return;
	}

	char* szMeshName = GetTrapMeshName(pGrenadeShot->GetItemID());
	if (!szMeshName) {
		return;
	}
	...
	if (pWeapon->Create(pMesh, pos, velocity, pGrenadeShot->GetItemID(), pC)) {
		Add(pWeapon);
	}
	else {
		delete pWeapon;
