...
//		MATCHCLIENT_DEFAULT_UDP_PORT, MTD_CharInfo* pCharInfo = NULL, int nTeam = 0);
	...
	void OnStageJoin(const MUID& uidChar, const MUID& uidStage, unsigned int nRoomNo, char* szStageName, bool isWar);
...
protected:
	...
	void OnGameLevelUp(const MUID& uidChar, const int nBounty, const int nCash, const int nItemID);
#else
...
#else
#	define ZPOSTCMD0(_ID)									if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); ZPostCommand(pC); }
#	define ZPOSTCMD1(_ID, _P0)								if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); ZPostCommand(pC); }
#	define ZPOSTCMD2(_ID, _P0, _P1)							if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); ZPostCommand(pC); }
#	define ZPOSTCMD3(_ID, _P0, _P1, _P2)					if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); ZPostCommand(pC); }
#	define ZPOSTCMD4(_ID, _P0, _P1, _P2, _P3)				if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); pC->AP(_P3); ZPostCommand(pC); }
#	define ZPOSTCMD5(_ID, _P0, _P1, _P2, _P3, _P4)			if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); pC->AP(_P3); pC->AP(_P4); ZPostCommand(pC); }
#	define ZPOSTCMD6(_ID, _P0, _P1, _P2, _P3, _P4, _P5)		if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); pC->AP(_P3); pC->AP(_P4); pC->AP(_P5); ZPostCommand(pC); }
#	define ZPOSTCMD7(_ID, _P0, _P1, _P2, _P3, _P4, _P5, _P6) if(!ZTddNeverPostCmd()) { MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); pC->AP(_P3); pC->AP(_P4); pC->AP(_P5); pC->AP(_P6); ZPostCommand(pC); }
#endif
