...
bool CheckAntiLeadStage()
{
	if (ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()) == false && ZGetGameTypeManager()->IsQuestChallengeDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()) == false && ZGetGame()->IsReplay() == false)
	{
		return true;
	}
	return false;
...
bool GetUserGradeIDColor(MMatchUserGradeID gid, MCOLOR& UserNameColor, char* sp_name)
{
	...
	if (gid == MMUG_DEVELOPER)
	{
		...
		UserNameColor = MCOLOR(51, 153, 102);
		return true;
	}
	else if (gid == MMUG_TRIAL) {
		UserNameColor = MCOLOR(127, 229, 255);
		return true;
	}
	else if (gid == MMUG_CHAT_LIMITED) {
		UserNameColor = MCOLOR(128, 128, 128);
		return true;
	}
	else if (gid == MMUG_STAR) {
		UserNameColor = MCOLOR(255, 201, 237);
		return true;
	}
#endif
...
void OnChangeBankai(MUID UID1, MUID UID2, bool b)
{
	if (UID1 != MUID(0, 0))
	{
		ZCharacter* pAttacker = (ZCharacter*)ZGetCharacterManager()->Find(UID1);
		if (pAttacker != NULL)
		{
			if (pAttacker->IsDie() == false)
			{
				if (pAttacker->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
				{
					// Check if new person in room
					if (b == true)
						return;
					...
					// Save Items
					if (ZGetGame()->m_pMyCharacter->GetItems()->GetItem((MMatchCharItemParts)MMCIP_HEAD)->IsEmpty() == false)
						ZGetGame()->m_pMyCharacter->GetItems()->SetOldItem((MMatchCharItemParts)MMCIP_HEAD, ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMCIP_HEAD)->GetDesc()->m_nID);
					else
						ZGetGame()->m_pMyCharacter->GetItems()->SetOldItem((MMatchCharItemParts)MMCIP_HEAD, 0);

					// Apply new Items
					ZGetGame()->m_pMyCharacter->ChangeWeapon(MMCIP_PRIMARY);
					if (ZGetGame()->m_pMyCharacter->IsMan())
					{
						ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_HEAD, 9201010);
						ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_MELEE, 135037);
					}
					else
					{
						ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_HEAD, 9201011);
						ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_MELEE, 135090);
					}
					ZGetGame()->m_pMyCharacter->ChangeWeapon(MMCIP_MELEE);

					// Aply New Head Mesh
					ZGetGame()->m_pMyCharacter->m_pVMesh->SetParts(eq_parts_head, ZGetGame()->m_pMyCharacter->GetItems()->GetItem(MMatchCharItemParts(MMCIP_HEAD))->GetDesc()->m_pMItemName->Ref().m_szMeshName);

					// Add Beserker Icon
					ZGetEffectManager()->AddBerserkerIcon(pAttacker);
					ZGetGame()->m_pMyCharacter->SetTagger(true);
					return;
				}
				else
				{
					...
					if (pAttacker->IsMan())
					{
						pAttacker->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_HEAD, 9201010);
						pAttacker->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_MELEE, 135037);
					}
					else
					{
						pAttacker->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_HEAD, 9201011);
						pAttacker->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_MELEE, 135090);
					}

					// Aply New Head Mesh
					pAttacker->m_pVMesh->SetParts(eq_parts_head, pAttacker->GetItems()->GetItem(MMatchCharItemParts(MMCIP_HEAD))->GetDesc()->m_pMItemName->Ref().m_szMeshName);
	...
	if (UID2 != MUID(0, 0))
	{
		ZCharacter* pVictim = (ZCharacter*)ZGetCharacterManager()->Find(UID2);
		if (pVictim != NULL)
		{
			if (pVictim->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
			{
				// Apply old items
				ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_HEAD, ZGetGame()->m_pMyCharacter->GetItems()->GetOldItem((MMatchCharItemParts)MMCIP_HEAD));
				ZGetGame()->m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)MMCIP_MELEE, ZGetGame()->m_pMyCharacter->GetItems()->GetOldItem((MMatchCharItemParts)MMCIP_MELEE));

				// Disable Beserker
				ZGetGame()->m_pMyCharacter->SetTagger(false);
				return;
			}
			return;
		}
...
bool ZGame::OnCommand_Immidiate(MCommand* pCommand)
{
	...
	*/
	...
	case MC_PEER_DAMAGECOUNTER:
	{
								  ...
								  ZCharacter* pVictim = (ZCharacter*)ZGetCharacterManager()->Find(pCommand->GetSenderUID());
								  if (pVictim == NULL)
									  break;
								  ...
								  else if (pVictim->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
								  {
									  pVictim->GetStatus().CheckCrc();
									  pVictim->GetStatus().Ref().nDamageTaken += Damage;
									  pVictim->GetStatus().MakeCrc();
								  }
	}
		break;

	case MC_PEER_VAMPIREHEALT:
	{
								 MUID AttackerUID;
								 pCommand->GetParameter(&AttackerUID, 0, MPT_UID);
								 ...
								 if (pCharacter->GetUID() == ZGetGame()->m_pMyCharacter->GetUID())
								 {
									 if (ZGetMyInfo()->GetClassJob() == MMCJ_ASSASIN)
									 {
										 // Store Taken HP/AP
										 pCharacter->GetStatus().CheckCrc();
										 pCharacter->GetStatus().Ref().nVampireTakenHP += 5;
										 pCharacter->GetStatus().Ref().nVampireTakenAP += 1;
										 pCharacter->GetStatus().MakeCrc();

										 // Insert Taken HP/AP
										 pCharacter->SetHP(pCharacter->GetHP() + 5);
										 pCharacter->SetAP(pCharacter->GetAP() + 1);
									 }
								 }
	}
		break;
	...
	case MC_MATCH_STAGE_ENTERBATTLE:
	{
									   ...
									   MEraseBlobArray(pBlob); //Nick Test
	}
		break;
	case MC_MATCH_STAGE_LEAVEBATTLE_TO_CLIENT:
	{
	...
	case MC_MATCH_RESPONSE_PEERLIST:
	{
		...
		break;
	case MC_PEER_PAYBACK:
	{
							char szName[MAX_CHARNAME_LENGTH]; //attacker name
							char szName2[MAX_CHARNAME_LENGTH]; //victim name
							char szMsg[512]; // msg saver.

							pCommand->GetParameter(szName, 0, MPT_STR, MAX_CHARNAME_LENGTH); // Parameter szName (attacker)
							pCommand->GetParameter(szName2, 1, MPT_STR, MAX_CHARNAME_LENGTH); // Parameter szName2[ (vicitim)

							ZCharacter* pChar = (ZCharacter*)ZGetCharacterManager()->Find(pCommand->GetSenderUID()); // get info from ZCharacter by Sender UID

							if (szName != "" && szName2 != "") // check if names are black
							{
								if (pCommand->GetSenderUID() == ZGetMyUID())	// if name is packback sender.
								{
									MUID m_uid;
									m_uid = MUID(0, 0);
									m_pMyCharacter->SetPayBackChar(m_uid);
								}
								sprintf(szMsg, "%s killed %s and got payback.", szName, szName2); // msg to everybody in game.

								if (szMsg != "") // just a check.
								{
									ZChatOutput(MCOLOR(0, 246, 145), szMsg); // send message.
									ZApplication::GetSoundEngine()->PlaySound("specials/payback"); //payback sound
								}
							}
	}
		break;
	case MC_PEER_HPAP:
	{
						 MUID VictimUID = pCommand->GetSenderUID();
						 MUID AttackerUID;
						 int nHP, nAP;
						 char szMsg[255];
						 ...
						 ZCharacter *pVictim = (ZCharacter*)ZGetCharacterManager()->Find(VictimUID);
						 if (pAttacker == NULL || pVictim == NULL)
							 break;

						 if (pVictim->GetUID() == ZGetGame()->GetMyCharacter()->GetUID())
						 {
							 ZItem* pItem = pAttacker->GetItems()->GetSelectedWeapon();
							 if (pItem != NULL)
							 {
								 const char *text = pItem->GetDesc()->m_pMItemName->Ref().m_szItemName;
								 if (text[0] == 'a' || text[0] == 'e' || text[0] == 'q' || text[0] == 'u' || text[0] == 'i' || text[0] == 'o' || text[0] == 'y' ||
									 text[0] == 'A' || text[0] == 'E' || text[0] == 'Q' || text[0] == 'U' || text[0] == 'I' || text[0] == 'O' || text[0] == 'Y')
								 {
									 sprintf(szMsg, "%s has destroy you with %s(%s)", pAttacker->GetUserNameA(), text, pItem->GetDesc()->GetWeaponString().c_str());
						 ...
						 else if (pAttacker->GetUID() == ZGetGame()->GetMyCharacter()->GetUID())
						 {
							 ZTransMsg(szMsg, MSG_GAME_WIN_FROM_WHO, 1, pVictim->GetUserNameA());
							 ZChatOutput(MCOLOR(0xFF80FFFF), szMsg);
						 }
						 else
						 {
							 ZTransMsg(szMsg, MSG_GAME_WHO_WIN_FROM_OTHER, 2, pAttacker->GetUserNameA(), pVictim->GetUserNameA());
							 ZChatOutput(MCOLOR(0xFF707070), szMsg);
						 }
	}
		break;
	case MC_PEER_VAMPIRE_DEATH:
	{
								  MUID VictimUID = pCommand->GetSenderUID();
								  MUID AttackerUID;
								  int nHP, nAP;
								  ...
								  if (pVictim->GetClassJob() == MMCJ_ASSASIN)
								  {
									  ZItem* pItem = pAttacker->GetItems()->GetSelectedWeapon();
									  if (pItem != NULL)
									  {
										  ...
										  if (text[0] == 'a' || text[0] == 'e' || text[0] == 'q' || text[0] == 'u' || text[0] == 'i' || text[0] == 'o' || text[0] == 'y' ||
											  text[0] == 'A' || text[0] == 'E' || text[0] == 'Q' || text[0] == 'U' || text[0] == 'I' || text[0] == 'O' || text[0] == 'Y')
										  {
											  if ((MMatchSex)pVictim->GetCharInfo()->nSex == MMS_MALE)
												  sprintf(szMsg, "%s has absorb %iHP/%iAP,His life is ended by %s with %s(%s)", pVictim->GetUserNameA(), nHP, nAP, pAttacker->GetUserNameA(), text, pItem->GetDesc()->GetWeaponString().c_str());
											  else
												  sprintf(szMsg, "%s has absorb %iHP/%iAP,His life is ended by %s with %s(%s)", pVictim->GetUserNameA(), nHP, nAP, pAttacker->GetUserNameA(), text, pItem->GetDesc()->GetWeaponString().c_str());
											  ZChatOutput(MCOLOR(0xFFCF2020), szMsg);
										  }
								  ...
								  {
									  ZChatOutput(MCOLOR(0, 246, 145), "Job Mistake found, Report this to the game owners"); // send message.
								  }
	}
		break;
	case MC_PEER_KILLSTREAK:
	{
							   ...
							   pCommand->GetParameter(&uid1, 0, MPT_UID);
							   pCommand->GetParameter(&uid2, 0, MPT_UID);
							   pCommand->GetParameter(szName, 2, MPT_STR, MAX_CHARNAME_LENGTH);
							   pCommand->GetParameter(&nKillStreakCount, 3, MPT_INT);
							   pCommand->GetParameter(&nOldKillStreakCount, 4, MPT_INT);
							   pCommand->GetParameter(szName2, 5, MPT_STR, MAX_CHARNAME_LENGTH);

							   if (nKillStreakCount == 0)
							   {
								   if (szName2 != "")
								   {
									   if (nOldKillStreakCount == 2)
										   sprintf(szMsg, "%s has stopped %s's (%i) multi kill.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 3)
										   sprintf(szMsg, "%s has stopped %s's (%i) killing spree.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 4)
										   sprintf(szMsg, "%s has stopped %s's (%i) Dominating.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 5)
										   sprintf(szMsg, "%s has stopped %s's (%i) Megakill.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 6)
										   sprintf(szMsg, "%s has stopped %s's (%i) Unstoppable.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 7)
										   sprintf(szMsg, "%s has stopped %s's (%i) Wicked Sick.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 8)
										   sprintf(szMsg, "%s has stopped %s's (%i) Monster Kill.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 9)
										   sprintf(szMsg, "%s has stopped %s's (%i) God Like.", szName2, szName, nOldKillStreakCount);
#ifndef _BANKAI
									   else if (nOldKillStreakCount == 10)
										   sprintf(szMsg, "%s has stopped %s's (%i) Beyond The God Like.", szName2, szName, nOldKillStreakCount);
#endif
									   else if (nOldKillStreakCount == 11)
										   sprintf(szMsg, "%s has stopped %s's (%i) Impossible.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 12)
										   sprintf(szMsg, "%s has stopped %s's (%i) Ultra Kill.", szName2, szName, nOldKillStreakCount);
									   else if (nOldKillStreakCount == 15)
										   sprintf(szMsg, "%s has stopped %s's (%i) Monster Kill.", szName2, szName, nOldKillStreakCount);
									   else
										   sprintf(szMsg, "%s %i-kill-streak has been ended by %s", szName2, nOldKillStreakCount, szName2);

#ifdef _BANKAI
									   if (nOldKillStreakCount == 10 || nOldKillStreakCount > 10)
									   {
										   sprintf(szMsg, "%s has stopped %s's the bankai (%i).", szName2, szName, nOldKillStreakCount);
										   OnChangeBankai(MUID(0, 0), uid2, false);
									   }
#endif
								   }
							   }
							   else if (nKillStreakCount == 2)
							   {
								   sprintf(szMsg, "%s has a multi kill!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/multikill");
							   }
							   else if (nKillStreakCount == 3)
							   {
								   sprintf(szMsg, "%s is on Killing Spree!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/killingspree");
							   }
							   else if (nKillStreakCount == 4)
							   {
								   sprintf(szMsg, "%s is on Dominating!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/dominating");
							   }
							   else if (nKillStreakCount == 5)
							   {
								   sprintf(szMsg, "%s is on MEGAKILL!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/megakill");
							   }
							   else if (nKillStreakCount == 6)
							   {
								   sprintf(szMsg, "%s is on Unstoppable!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/unstoppable");
							   }
							   else if (nKillStreakCount == 7)
							   {
								   sprintf(szMsg, "%s is on Wicked Sick!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/WickedSick");
							   }
							   else if (nKillStreakCount == 8)
							   {
								   sprintf(szMsg, "%s is on Monster Kill!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/monsterkill");
							   }
							   else if (nKillStreakCount == 9)
							   {
								   sprintf(szMsg, "%s is on God Like!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/godlike");
							   }
							   else if (nKillStreakCount == 10)
							   {
#ifdef _BANKAI
								   // Bankai Message
								   sprintf(szMsg, "%s the bankai has been spawned!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/Bankai");

								   //Change to bankai
								   OnChangeBankai(uid1, MUID(0, 0), false);
#else
								   sprintf(szMsg, "%s is on Beyond The God Like!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/BeyondGodlike");
#endif
							   }
							   else if (nKillStreakCount == 11)
							   {
								   sprintf(szMsg, "%s is on Impossible!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/impossible");
							   }
							   else if (nKillStreakCount == 12)
							   {
								   sprintf(szMsg, "%s is on Ultra Kill!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/ultrakill");
							   }
							   else if (nKillStreakCount == 15)
							   {
								   sprintf(szMsg, "%s is on Monster Kill!(%i kills)", szName, nKillStreakCount);
								   ZApplication::GetSoundEngine()->PlaySound("specials/monsterkill");
							   }
							 ...
							 if ((nTeam == MMT_ALL) || (nTeam == MMT_SPECTATOR))
							 {
								 if (!ZGetGameClient()->GetRejectNormalChat() || (strcmp(pChar->GetUserName(), ZGetMyInfo()->GetCharName()) == 0))
								 {
									 ...
									 if (bSpUser)
									 {
										 ...
										 {
#ifndef _UNMASK
											 sprintf(szTemp, "^0[Dead]%s : %s", sp_name, szMsg);
#else
	...
	*/
	...
	case MC_MATCH_NOLEAD:
	{
							...
							MTD_ShotInfo* pInfo = (MTD_ShotInfo*)MGetBlobArrayElement(pBlob, 0);
							if (pInfo == NULL)
								break;

							if (MUID(pInfo->nAttackerHighID, pInfo->nAttackerLowID) == MUID(0, 0))
								break;

							if (m_pMyCharacter)
							{
								if (MUID(pInfo->nVictimHighID, pInfo->nVictimLowID) == ZGetGameClient()->GetPlayerUID())
								{
									ZCharacter* pAttacker = (ZCharacter*)ZGetCharacterManager()->Find(MUID(pInfo->nAttackerHighID, pInfo->nAttackerLowID));

									if (!ZGetGame()->GetMatch()->IsTeamPlay() || ((m_pMyCharacter->IsTeam(pAttacker) && ZGetGame()->GetMatch()->GetTeamKillEnabled()) || !m_pMyCharacter->IsTeam(pAttacker)))
									{
										if (pInfo->nItemPart == MMCIP_END)
										{
											break;
										}
										else
										{
											ZItem *pItem = (MMCIP_MELEE <= pInfo->nItemPart && pInfo->nItemPart <= MMCIP_SECONDARY) ? pAttacker->GetItems()->GetItem((MMatchCharItemParts)pInfo->nItemPart) : 0;
											if (!pItem || !pItem->GetDesc())
												break;

											float fRatio = pItem->GetPiercingRatio(pItem->GetDesc()->m_nWeaponType.Ref(), (RMeshPartsType)pInfo->nPartType);
											float fDamage = CalcActualDamage(pAttacker, m_pMyCharacter, (float)pItem->GetDesc()->m_nDamage.Ref());
											if (fDamage == 0)
												break;

											if (pInfo->nMeleeType == 1)
											{
												if (pInfo->nWeaponType == MWT_DAGGER || pInfo->nWeaponType == MWT_KATANA || pInfo->nWeaponType == MWT_DOUBLE_KATANA || pInfo->nWeaponType == MWT_DUAL_DAGGER || pInfo->nWeaponType == MWT_GREAT_SWORD
													|| pInfo->nWeaponType == MWT_KATANA2 || pInfo->nWeaponType == MWT_KATANA3 || pInfo->nWeaponType == MWT_KATANA4 || pInfo->nWeaponType == MWT_AXE || pInfo->nWeaponType == MWT_BASEBALL
													|| pInfo->nWeaponType == MWT_FIGHTER || pInfo->nWeaponType == MWT_SWORD2 || pInfo->nWeaponType == MWT_DOUBLE_KATANA2 || pInfo->nWeaponType == MWT_DOUBLE_KATANA3)
												{
													if (m_pMyCharacter->IsGuard() == false || DotProduct(m_pMyCharacter->GetDirection(), D3DXVECTOR3(pInfo->fDirx, pInfo->fDiry, pInfo->fDirz)) >= 0.0f)
													{
														m_pMyCharacter->OnDamaged(pAttacker, pAttacker->GetPosition(), (ZDAMAGETYPE)pInfo->nDamageType, (MMatchWeaponType)pInfo->nWeaponType, fDamage, fRatio, pInfo->nMeleeType);
													}
													else
													{
														m_pMyCharacter->OnMeleeGuardSuccess();
													}
												}
												else
												{
													char szErr[255];
													sprintf(szErr, "[NO-LEAD] : (ERROR) MeleeType %d | WeaponType %d", pInfo->nMeleeType, pInfo->nWeaponType);
													ZChatOutput(szErr);
												}
											}
											else
											{
												ZDAMAGETYPE nDamageType = ZD_BULLET;
												if (pInfo->nPartType == eq_parts_head)
												{
													nDamageType = ZD_BULLET_HEADSHOT;
													if (pItem->GetDesc()->m_nWeaponType.Ref() == MWT_SNIFER && pInfo->nWeaponType == MWT_SNIFER)
													{
														fDamage = 9999;
													}
												}
												m_pMyCharacter->OnDamaged(pAttacker, pAttacker->GetPosition(), (ZDAMAGETYPE)nDamageType, (MMatchWeaponType)pInfo->nWeaponType, fDamage, fRatio);
											}
										}
									}
								}
...
void ZGame::OnExplosionGrenade(MUID uidOwner, rvector pos, float fDamage, MMatchCharItemParts m_nItemParts, float fRange, float fMinDamage, float fKnockBack, MMatchTeam nTeamID)
{
...
#define MAX_DMG_RANGE	50.f	// 반경이만큼 까지는 최대 데미지를 다 먹는다
		...
		// Grenade No Lead sucks your balls and toke from me almost 1 hour free time thx lol
		//if (CheckAntiLeadStage())
		//	((ZCharacter*) (pTarget))->OnDamagedAPlayer(pOwnerCharacter, pTarget->GetUID(), MWT_FRAGMENTATION, eq_parts_chest, m_nItemParts, ZD_EXPLOSION, -1);
		//else
...
int ZGame::SelectSlashEffectMotion(ZCharacter* pCharacter)
{
	...
	if (pCharacter->IsMan()) {
		...
		else if (nType == MWT_KATANA1)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_KATANA2)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_KATANA3)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_KATANA4)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_AXE)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_BASEBALL)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_FIGHTER)	return SEM_ManSlash1 + nAdd;
		else if (nType == MWT_SWORD2)	return SEM_ManGreatSwordSlash1 + nAdd;
		else if (nType == MWT_DOUBLE_KATANA2)	return SEM_ManDoubleSlash1 + nAdd;
		else if (nType == MWT_DOUBLE_KATANA3)	return SEM_ManDoubleSlash1 + nAdd;
		...
		else if (nType == MWT_KATANA1)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_KATANA2)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_KATANA3)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_KATANA4)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_AXE)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_BASEBALL)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_FIGHTER)	return SEM_WomanSlash1 + nAdd;
		else if (nType == MWT_SWORD2)	return SEM_WomanGreatSwordSlash1 + nAdd;
		else if (nType == MWT_DOUBLE_KATANA2)	return SEM_WomanDoubleSlash1 + nAdd;
		else if (nType == MWT_DOUBLE_KATANA3)	return SEM_WomanDoubleSlash1 + nAdd;
	}
...
void ZGame::OnPeerShot_Melee(const MUID& uidOwner, float fShotTime)
{
	...
	for (ZObjectManager::iterator itor = m_ObjectManager.begin(); itor != m_ObjectManager.end(); ++itor)
	{
		...
		if (CheckAntiLeadStage() && !pVictim->IsNPC())
			((ZCharacter*)(pVictim))->OnDamagedAPlayer(pAttacker, pVictim->GetUID(), pItemDesc->m_nWeaponType.Ref(), eq_parts_chest, MMCIP_MELEE, ZD_MELEE, cm);
		else
			pVictim->OnDamaged(pAttacker, pAttacker->GetPosition(), ZD_MELEE, pItemDesc->m_nWeaponType.Ref(), fActualDamage, fRatio, cm);
...
{
	//### 이 함수를 수정하면 똑같이 아래의 InRanged()에도 적용해 주어야 합니다. ###
...
void ZGame::OnPeerShot_Range_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, const rvector& to, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, MMatchCharItemParts sel_type)
{
	MMatchItemDesc *pDesc = pItem->GetDesc();
	...
	if (CheckAntiLeadStage() && !pickinfo.pObject->IsNPC())
		((ZCharacter*)(pickinfo.pObject))->OnDamagedAPlayer(pOwner, pickinfo.pObject->GetUID(), pDesc->m_nWeaponType.Ref(), pickinfo.info.parts, sel_type, dt, -1);
	else
...
void ZGame::OnPeerShot_Shotgun(ZItem *pItem, ZCharacter* pOwnerCharacter, float fShotTime, const rvector& pos, const rvector& to, const MMatchCharItemParts nItemParts)
{
...
#define SHOTGUN_DIFFUSE_RANGE	0.1f
	...
	for (int i = 0; i<SHOTGUN_BULLET_COUNT; i++)
	{
		...
		OnPeerShotgun_Damaged(pOwnerCharacter, fShotTime, pos, dir, pickinfo, dwPickPassFlag, v1, v2, pItem, BulletMarkNormal, bBulletMark, nTargetType, bHitEnemy, nItemParts);
...
void ZGame::OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector& dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy, MMatchCharItemParts nItemParts)
{
	ZCharacter *pTargetCharacter = ZGetGameInterface()->GetCombatInterface()->GetTargetCharacter();
	...
	if (CheckAntiLeadStage() && !pickinfo.pObject->IsNPC())
		((ZCharacter*)(pickinfo.pObject))->OnDamagedAPlayer(pOwner, pickinfo.pObject->GetUID(), pDesc->m_nWeaponType.Ref(), pickinfo.info.parts, nItemParts, dt, -1);
	else
		pickinfo.pObject->OnDamaged(pOwner, pOwner->GetPosition(), dt, pDesc->m_nWeaponType.Ref(), fActualDamage, fRatio);
...
void ZGame::OnPeerShot(const MUID& uid, float fShotTime, const rvector& pos, const rvector& to, const MMatchCharItemParts sel_type)
{
	...
	*/
	...
	rvector _pos = pos;
	MUID _uid = uid;
	if ((wtype >= MWT_PISTOL) && (wtype <= MWT_SNIFER))
		ZGetCombatInterface()->GetRadar()->OnAttack(_pos, _uid);
#endif

	if (wtype == MWT_SHOTGUN)
	{
		OnPeerShot_Shotgun(pItem, pOwnerCharacter, fShotTime, pos, to, sel_type);
		return;
	}
	else {
...
void ZGame::OnPeerDieMessage(ZCharacter* pVictim, ZCharacter* pAttacker)
{
	...
	const char *szAnonymous = "Anonymouse";

	char szVictim[256];
	strcpy(szVictim, pVictim ? pVictim->GetUserAndClanName() : szAnonymous);

	char szVictim2[256];
	strcpy(szVictim2, pVictim ? pVictim->GetUserName() : szAnonymous);

	char szAttacker[256];
	strcpy(szAttacker, pAttacker ? pAttacker->GetUserAndClanName() : szAnonymous);

	char szAttacker2[256];
	strcpy(szAttacker2, pAttacker ? pAttacker->GetUserName() : szAnonymous);
	...
	else if (pAttacker == m_pMyCharacter)
	{
		...
		char szOutput[256] = "";
		m_pMyCharacter->GetStatus().CheckCrc();
		m_pMyCharacter->GetStatus().Ref().nKillStreakCount++;
		m_pMyCharacter->GetStatus().MakeCrc();

		if (m_pMyCharacter->GetStatus().Ref().nKillStreakCount > 2 && m_pMyCharacter->GetStatus().Ref().nKillStreakCount < 15)
			ZPostKillStreak(pAttacker->GetUID(), pVictim->GetUID(), szAttacker2, m_pMyCharacter->GetStatus().Ref().nKillStreakCount, 0, "");

		//payback system
		if (m_pMyCharacter->GetPayBackChar() == pVictim->GetUID())
			ZPostPayback(szAttacker2, szVictim2);

	}
	else if (pVictim == m_pMyCharacter)
	{
		//Disable Sniper Scoop
		...
		//kill Streak
		if (m_pMyCharacter->GetStatus().Ref().nKillStreakCount > 2)
		{
			ZPostKillStreak(pAttacker->GetUID(), pVictim->GetUID(), szVictim2, 0, m_pMyCharacter->GetStatus().Ref().nKillStreakCount, szAttacker2);
		}
		...
		//payback system
		m_pMyCharacter->SetPayBackChar(pAttacker->GetUID());

#ifdef _ASSASIN_SKILL
		if (pVictim->GetClassJob() == MMCJ_ASSASIN)
		{
			ZPostVampireDeath(pAttacker->GetUID(), pVictim->GetStatus().Ref().nVampireTakenHP, pVictim->GetStatus().Ref().nVampireTakenAP);
		}
		else
		{
			ZPostHPAP(pAttacker->GetUID(), pAttacker->GetHP(), pVictim->GetAP());
		}
#endif
...
bool ZGame::FilterDelayedCommand(MCommand *pCommand)
{
	...
	case MC_QUEST_PEER_NPC_ATTACK_MELEE:	// npc 칼질
		...
		// 룰에서도 필터링이 안되면 이건 delayed command가 아니다
	default:
	{
			   if (m_Match.GetRule())
			   {
				   fDelayTime = m_Match.GetRule()->FilterDelayedCommand(pCommand);
				   if (fDelayTime > 0)
					   break;
			   }
...
#ifdef _QUEST
	if (ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()) || ZGetGameTypeManager()->IsQuestChallengeDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()))
	{
		if (pAttacker->GetTeamID() == pTarget->GetTeamID())
		{
			return false;
		...
		case WORLDITEM_PORTAL_ID:
			{
				...
				ZPostQuestRequestMovetoPortal(ZGetQuest()->GetGameInfo()->GetCurrSectorIndex());
			}
			break;
	};
}
...
bool ZGame::OnRuleCommand(MCommand* pCommand)
{
	...
	if (m_Match.IsQuestChallengeDrived())
	{
...
void ZGame::OnExplosionDynamite(MUID uidOwner, rvector pos, float fDamage, float fRange, float fKnockBack, MMatchTeam nTeamID, MMatchCharItemParts nItemParts)
{
	ZObject* pTarget = NULL;
	...
	for (ZObjectManager::iterator itor = m_ObjectManager.begin(); itor != m_ObjectManager.end(); ++itor)
	{
		...
		// NO Lead on Dynamitye not needed i think
		//if (CheckAntiLeadStage())
		//	((ZCharacter*) (pTarget))->OnDamagedAPlayer(pOwnerCharacter, pTarget->GetUID(), MWT_DYNAMITYE, eq_parts_chest, nItemParts, ZD_EXPLOSION, -1);
		//else
