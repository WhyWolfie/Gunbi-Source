...
void ZWeaponGrenade::Explosion()
{
	...
	ZGetGame()->OnExplosionGrenade(m_uidOwner, v, m_fDamage, m_nItemParts, 400.f, .2f, 1.f, m_nTeamID);
...
void ZWeaponDynamite::Explosion()
{
	...
	ZGetGame()->OnExplosionDynamite(m_uidOwner, m_Position, m_fDamage, 400.f, 1.f, m_nTeamID, m_nItemParts);
...
{
	if (!Init(pMesh, pos, velocity, nItemId, pOwner)) {
		return false;
	}
