...
#ifdef _EXTRASHOPS
	ZGetShop()->SetShopMode(SHOP_NORMAL);
#endif
...
BEGIN_IMPLEMENT_LISTENER(ZGetCashShopCallerButtonListener, MBTN_CLK_MSG)
	ZGetShop()->SetShopMode(SHOP_CASH);
	ZGetGameInterface()->ShowShopDialog();
END_IMPLEMENT_LISTENER()
...
BEGIN_IMPLEMENT_LISTENER(ZGetChannelList, MBTN_CLK_MSG)
	MButton *pButton = (MButton*)pWidget;
	int nIndexInGroup = pButton->GetIndexInGroup();
	_ASSERT(0<=nIndexInGroup && nIndexInGroup < 5);
	...
	switch(nIndexInGroup) {
		...
		case 4 : nChannelType = MCHANNEL_TYPE_WAR; break;
...
BEGIN_IMPLEMENT_LISTENER(ZGetWarGameButtonListener, MBTN_CLK_MSG)
	ZPostWarRequestJoinGame();
END_IMPLEMENT_LISTENER();
