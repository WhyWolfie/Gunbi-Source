...
bool ZGameInput::OnEvent(MEvent* pEvent)
{
	...
	if (pEvent->nMessage != MWM_MOUSEMOVE)
		ZGetGameClient()->SetLastKeyTime(timeGetTime());
	#endif
...
//						ZApplication::GetGameInterface()->GetScreenDebugger()->SwitchDebugInfo();
			...
			case VK_ESCAPE:		// 메뉴를 부르거나 kick player를 취소한다
				...
				return true;
			case 'M' : 
				if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_SOLO || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_GLADIATOR_SOLO || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_TRAINING || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_BERSERKER || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DROPGUNGAME || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SNIPERMODE || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CLASSICMODE)
				{
