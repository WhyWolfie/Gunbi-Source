	...
	if ((pObject->GetInitialized()) && (pObject->IsVisible()))
	{
		...
		if(pObject->IsRendered())
		{
			if ((MIsExactlyClass(ZNetCharacter, pObject)) ||
				...
				(MIsExactlyClass(ZActor, pObject)) ||
				(MIsExactlyClass(ZActorWithFSM, pObject)) ||
				(MIsExactlyClass(ZDummyNPC, pObject)))
			{
...
ZActorBase* ZObjectManager::GetNPCObject(const MUID& uidNPC)
{
	return (ZActorBase*)m_NPCObjectMap.Find(uidNPC);
}