...
LOADING_DRAW_MODE ZInitialLoading::drawMode = MODE_FULLSCREEN;
...
void	ZInitialLoading::Draw( LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_ )
{
	...
	//	if( mode_ == MODE_FADEOUT )
		{
			...
			if (drawMode == MODE_FULLSCREEN)
				DrawBar(0.0, 0.0, (float)RGetScreenWidth(), (float)RGetScreenHeight(), mPercentage * 0.01f);
			else
			#endif
	...
	// 청소년 자율 규제 적용안(쓰벌쓰벌쓰벌...)...
#if defined(LOCALE_KOREA) || defined(_WHOLE_SCREEN_LOADING)
		#ifdef _WHOLE_SCREEN_LOADING
		if (mpDC && (drawMode == MODE_FULLSCREEN))
		#else
		if (mpDC)
		#endif
		{
