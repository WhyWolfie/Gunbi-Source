...
struct ZScoreBoardItem : public CMemPoolSm<ZScoreBoardItem>{
	...
	ZScoreBoardItem(const MUID& _uidUID, char* _szLevel, char *_szName, char *_szClan, int _nTeam, bool _bDeath, int _nExp, int _nKills, int _nDeaths, int _nPing, int _nDTLastWeekGrade, bool _bMyChar, bool _bGameRoomUser, int _nClassJob, int _nTotalKills, int _nTotalDeaths, bool _bCommander = false)
	{
		uidUID=_uidUID;
		...
		nClassJob = _nClassJob;
		nTotalKills = _nTotalKills;
		nTotalDeaths = _nTotalDeaths;
	}
	ZScoreBoardItem() {
		bSpColor = false;
		SpColor = MCOLOR(0,0,0);
...
bool ZCombatInterface::OnCreate()
{
	...
	if ( pWidget)
	{
		...
		if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_WAR) {
			pWidget->Show(false);
		}
	}
...
void ZCombatInterface::OnDraw(MDrawContext* pDC)
{
	...
	m_ScreenDebugger.DeveloperDebugInfo(pDC);
#endif
	...
	if(IsShowUI())				// 모든 UI 감추기... by kam 20081020
	{
		...
		if (!m_bSkipUIDrawByRule)
		{
			...
			if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DROPGUNGAME)
			{
		...
		if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_WAR) {
			...
			ZRuleWarMode* pWar = (ZRuleWarMode*)ZGetGameInterface()->GetGame()->GetMatch()->GetRule();
			if (pWar) {

				ZDummyNPC* pTower_red = pWar->GetTower(MMT_RED);
				ZDummyNPC* pTower_blue = pWar->GetTower(MMT_BLUE);

				if (pTower_blue && pTower_red) {
					...
					nWidth = (int)((float)pTower_red->GetActualHP() / pTower_red->GetActualMaxHP() * fLength);
					pDC->FillRectangleW((193.0f + 163.0f)*fRx - nWidth, fPosy, nWidth, fHeight);

					nWidth = (int)((float)pTower_blue->GetActualHP() / pTower_blue->GetActualMaxHP() * fLength);
					...
					nWidth = (int)((float)pTower_red->GetActualAP() / pTower_red->GetActualMaxAP() * fLength);
					pDC->FillRectangleW((218.0f + 150.0f)*fRx - nWidth, 37.0f*fRy, nWidth, 5.0f*fRy);

					nWidth = (int)((float)pTower_blue->GetActualAP() / pTower_blue->GetActualMaxAP() * fLength);
					pDC->FillRectangleW(432.0f*fRx, 37.0f*fRy, nWidth, 5.0f*fRy);

					// Name 
					pDC->SetColor(MCOLOR(255,255,255));
					TextRelative(pDC, 0.34f, 0.026f, "Red", true);
					TextRelative(pDC, 0.66f, 0.026f, "Blue", true);

					MBitmap* pBitmap = MBitmapManager::Get("DuelTournament_score.tga");;
					if (pBitmap)
					{
						pDC->SetBitmap(pBitmap);
						if (RGetIsWidthScreen())
...
void ZCombatInterface::DrawMyHPAPPont(MDrawContext* pDC)
{
	...
	if ( (ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUEL) || ( !pCharacter->IsObserverTarget()))
	{
		TextRelative(pDC,100.f/800.f, 23.f/600.f,hp_buffer);
		TextRelative(pDC,100.f/800.f, 50.f/600.f,ap_buffer);
	}
}
...
void ZCombatInterface::Update(float fElapsed)
{
	...
	if ( m_bShowResult)
	{
		if ( timeGetTime() > m_nReservedOutTime)
		{
			if(ZGetGameClient()->IsLadderGame() || ZGetGameClient()->IsDuelTournamentGame() || ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_WAR)
...
void ZCombatInterface::SetPickTarget(bool bPick, ZCharacter* pCharacter)
{
	...
	if (bPick)
	{
		...
		if (bFriend == false) 
		{
			...
			m_pWeaponScreenEffect->SetState(ZCS_PICKENEMY);
			m_pTargetLabel->SetTextColor(0xffff0000);
		}
	...
	else
	{
		...
		m_pWeaponScreenEffect->SetState(ZCS_NORMAL);
...
void ZCombatInterface::DrawScoreBoardCW(MDrawContext* pDC)
{
	...
	// player
	// 화면의 xy 상대위치 ( 0~1 빺봤을때 ) 는 0.25 , 0.361
	y = 0.343f;
	...
	for (i = items.begin(); i != items.end(); i++)
	{
		...
		if (pItem->bSpColor)	// 특수한 유저라면..고유의 컬빉를 가지고 있다.
		{
			if (!pItem->bDeath)
...
void ZCombatInterface::DrawScoreBoardSolo(MDrawContext* pDC){
	...
	else if (ZGetGameTypeManager()->IsSurvivalOnly(ZGetGame()->GetMatch()->GetMatchType())) 	// 서바이벌 모드일 경우
	{
		...
		// 서바이벌엔 자코가 없으므빺 보스가 나오더라도 잔여 npc 수는 표시 가능하다
		sprintf(szText, "%s : %d", ZMsg(MSG_WORD_REMAINNPC), nNPCKilled);
		TextRelative(pDC, x, y, szText);
		y -= linespace2;
	}
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL)
	{
		if (ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_PLAY)				// 플빘이 중이라면
		{
			DWORD dwTime = ZGetGame()->GetMatch()->GetRemaindTime();
			DWORD dwLimitTime = ZGetGameClient()->GetMatchStageSetting()->GetStageSetting()->nLimitTime;
			if (dwLimitTime != 99999)
	...
	// 라운드를 기다빦야 하는 종뺲라면 라운드 표시 아니면 시간 표시
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		//alex
		// ZGetGameClient()->GetPremiumGradeIDColor(pgid,_color,"");
		ZGetGame()->GetUserNameColor(pCharacter->GetUID(), _color, "");
...
void ZCombatInterface::GameCheckPickCharacter()
{
	...
	if(pMyChar && pMyChar->m_pVMesh) {
		...
		if( (type==eq_wd_katana) || (type==eq_wd_grenade) || (type==eq_ws_dagger) || (type==eq_wd_dagger) 
			|| (type==eq_wd_item) || (type==eq_wd_sword) || (type==eq_wd_blade) || (type==eq_wd_axe)  || (type==eq_wd_baseball)
			|| (type==eq_wd_katana1) || (type==eq_wd_katana2) || (type==eq_wd_katana3) || (type==eq_wd_katana4) || (type==eq_wd_fighter)
			|| (type==eq_wd_sword2) || (type==eq_wd_blade2) || (type==eq_wd_blade3))
		{
			bPick = false;
		}
...
void ZCombatInterface::OnGadget(MMatchWeaponType nWeaponType)
{
	if (m_pWeaponScreenEffect) 
		m_pWeaponScreenEffect->OnGadget(nWeaponType);
	m_CrossHair.Show(false);
}
