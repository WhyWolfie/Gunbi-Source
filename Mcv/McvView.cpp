...
//		SetLight(pVMesh,2,rvector(0,150,200));
		...
		if(g_effect_model) {
			...
			if(!pVMesh->GetMesh()->m_bEffectSort)//이펙트모델은 그냥 놔둔다.
...
*/
		...
		if (g_bDrawBBox) {
			draw_box(&g_matWorld, (D3DXVECTOR3)pVMesh->GetBoundMax(), (D3DXVECTOR3)pVMesh->GetBoundMin(), 0xffffffff);
		}
	}
...
void CMcvView::LoadAniFile(char* FileName)
{
	...
	if( pAni && pVMesh ) {
		pVMesh->GetMesh()->SetAnimation(pAni_);//이전것소거..
...
void CMcvView::OnMapObjPos()
{
	...
	if(pVMesh) {
		g_map_obj_pos = pVMesh->GetBoundMin() + (pVMesh->GetBoundMax() - pVMesh->GetBoundMin())/2.f;
...
void CMcvView::OnPartscolor()
{
	...
	if( dlg.DoModal()==IDOK ) {
		...
		if(pVMesh) {
			...
			RMeshNode* pMNode = pVMesh->GetTMesh()[type];
