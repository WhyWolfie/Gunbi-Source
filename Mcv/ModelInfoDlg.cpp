...
void  CModelInfoDlg::Begin()
{
	...
	if(pVMesh) {
		...
		RMtrlMgr* pMtrlMgr = &pVMesh->GetMesh()->m_mtrl_list_ex;

		int mtrl_size = pMtrlMgr->size();

int i = 0;		for( i=0;i<mtrl_size;i++) {
...
void CModelInfoDlg::OnBnClickedButtonMeshnodeColor()
{
	...
	if( dlg.DoModal()==IDOK ) {
		...
		RMeshNode* pMeshNode = pVMesh->GetMesh()->GetMeshData((char*)str.operator const char*());
