...
//				pCommand->GetParameter(&nTimeStamp, 0, MPT_UINT);
		...
		case MC_WAR_GAME_REQUEST_JOINGAME:
			{
				ResponseWarJoinGame(pCommand->GetSenderUID());
			}
			break;

		case MC_WAR_NPC_DEAD: 
			{
				MUID uidAttacker, uidNpc;

				pCommand->GetParameter(&uidAttacker, 0, MPT_UID);
				pCommand->GetParameter(&uidNpc, 1, MPT_UID);

				RequestWarNPCDead(uidAttacker, uidNpc);
			}
			break;

		case MC_WAR_SYNC_HPAP:
			{
				MUID uidNpc;
				int nHP, nAP;
				...
				UpdateWarNPCHpAp(pCommand->GetSenderUID(), uidNpc, nHP, nAP);
			}
			break;
