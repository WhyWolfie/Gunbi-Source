...
void MMatchServer::OnResponseBuyQuestItem( const MUID& uidSender, const unsigned long int nItemID, const int nItemCount )
{
	...
	if (pPlayer->GetCharInfo()->m_ClassJob == MMCJ_ASSASIN)
	{
		if (!MGetMatchShop1()->IsSellItem(pQuestItemDesc->m_nItemID))
		{
			mlog("MMatchServer::OnRequestBuyQuestItem - %d는 상점에서 판매되고 있는 아이템이 아님.\n", pQuestItemDesc->m_nItemID);
			return;
		}
	}
	else if (pPlayer->GetCharInfo()->m_ClassJob == MMCJ_COWBOY)
	{
		if (!MGetMatchShop2()->IsSellItem(pQuestItemDesc->m_nItemID))
		{
			mlog("MMatchServer::OnRequestBuyQuestItem - %d는 상점에서 판매되고 있는 아이템이 아님.\n", pQuestItemDesc->m_nItemID);
			return;
		}
	}
	else if (pPlayer->GetCharInfo()->m_ClassJob == MMCJ_DEFFENDER)
	{
		if (!MGetMatchShop3()->IsSellItem(pQuestItemDesc->m_nItemID))
		{
			mlog("MMatchServer::OnRequestBuyQuestItem - %d는 상점에서 판매되고 있는 아이템이 아님.\n", pQuestItemDesc->m_nItemID);
			return;
		}
	}
	else if (pPlayer->GetCharInfo()->m_ClassJob == MMCJ_SOLDIER)
	{
		if (!MGetMatchShop4()->IsSellItem(pQuestItemDesc->m_nItemID))
		{
			mlog("MMatchServer::OnRequestBuyQuestItem - %d는 상점에서 판매되고 있는 아이템이 아님.\n", pQuestItemDesc->m_nItemID);
			return;
		}
