...
MUIDRefCache::iterator MMatchStage::RemoveObject(const MUID& uid)
{
	...
	MMatchChannel* pChannel = MMatchServer::GetInstance()->FindChannel(pObj->GetChannelUID());
	if (pChannel && pChannel->GetRuleType() != MCHANNEL_RULE_WAR) {
	...
	case STAGE_STATE_RUN:
		{
			if (m_pRule) 
			{
				...
				if (m_pRule->Run() == false) 
				{
					...
					MMatchChannel* pChannel = MMatchServer::GetInstance()->FindChannel(GetOwnerChannel());
					if (pChannel && pChannel->GetChannelType() == MCHANNEL_RULE_WAR && GetState() != STAGE_STATE_CLOSE) {
						ChangeState(STAGE_STATE_STANDBY);
					}
					else {
...
MMatchRule* MMatchStage::CreateRule(MMATCH_GAMETYPE nGameType)
{
	...
	case MMATCH_GAMETYPE_SNIPERMODE:
		{
			return (new MMatchRuleTeamDeath(this)); // No Need for his own Rule file
		}
		break;
	case MMATCH_GAMETYPE_CLASSICMODE:
		{
			return (new MMatchRuleSoloDeath(this)); // No Need for his own Rule file
		}
		break;
...
bool MMatchStage::StartGame( const bool bIsUseResourceCRC32CacheCheck )
{
	...
	bool isBlitzKrieg = m_StageSetting.GetGameType() == MMATCH_GAMETYPE_BLITZKRIEG || m_StageSetting.GetGameType() == MMATCH_GAMETYPE_WAR;

	// 인원수 체크
	int nPlayer = GetCountableObjCount();
	if (nPlayer > m_StageSetting.GetMaxPlayers() && !isBlitzKrieg)
	{
		char szMsg[ 256];
	...
	if (m_StageSetting.IsQuestDrived())
	{
		if (nPlayer > STAGE_QUEST_MAX_PLAYER)
		{
			char szMsg[256];
			sprintf(szMsg, "%s%d", MTOK_ANNOUNCE_PARAMSTR, MERR_PERSONNEL_TOO_MUCH);
...
void MMatchStage::OnApplyTeamBonus(MMatchTeam nTeam)
{
	...
	if (!m_StageSetting.IsTeamExtremeGame())		// 으아아악 이런 갓뎀코드를 만들다니 -_-;
	{
		for (MUIDRefCache::iterator i=GetObjBegin(); i!=GetObjEnd(); i++) 
		{
			MMatchObject* pObj = (MMatchObject*)(*i).second;
...
bool MMatchStage::IsValidMap( const char* pMapName )
{
	...
	if (MGetGameTypeMgr()->IsQuestChallengeDerived(GetStageSetting()->GetGameType())) {
		return true;
	}
	...
	bool IsDule = MMATCH_GAMETYPE_DUEL == m_StageSetting.GetGameType();
	bool IsCTF = MMATCH_GAMETYPE_CTF == m_StageSetting.GetGameType();
...
#ifndef _SUICIDE_QUICK
	MMatchStageSuicide SuicideUser( uidUser, dwExpireTime + 10000 );
#else
	MMatchStageSuicide SuicideUser( uidUser, dwExpireTime);
#endif
