		...
		case MC_MATCH_RESPONSE_LOGIN:
			{
				...
				pCommand->GetParameter(&nCashCoins,		6, MPT_INT);
				...
				OnResponseMatchLogin(pCommand->GetSenderUID(), nResult, szServerName, MMatchServerMode(nServerMode), szAccountID, MMatchUserGradeID(nUGradeID), 
#ifdef _CASHCOINS
					MMatchPremiumGradeID(nPGradeID),nCashCoins, 
#endif
...
void MMatchClient::ReplaceObjCache(MMatchObjCache* pCache)
{
	...
	pScanCache->SetInfo(pCache->GetUID(), pCache->GetName(), pCache->GetClanName(), pCache->GetLevel(), pCache->GetUGrade(), pCache->GetPGrade(), pCache->GetClassJob(), pCache->GetRank(), pCache->GetKillCount(), pCache->GetDeathCount(), pCache->GetDTGrade() );
...
void MMatchClient::UpdateObjCache(MMatchObjCache* pCache)
{
	...
	pNewCache->SetInfo(pCache->GetUID(), pCache->GetName(), pCache->GetClanName(), pCache->GetLevel(), pCache->GetUGrade(), pCache->GetPGrade(), pCache->GetClassJob(), pCache->GetRank(), pCache->GetKillCount(), pCache->GetDeathCount(), pCache->GetDTGrade());
