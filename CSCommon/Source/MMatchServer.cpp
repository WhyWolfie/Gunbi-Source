...
#define FILENAME_SHOP1					"shop1.xml"
#define FILENAME_SHOP2					"shop2.xml"
#define FILENAME_SHOP3					"shop3.xml"
#define FILENAME_SHOP4					"shop4.xml"
...
#define FILENAME_BLITZKRIEG_XML			"blitzkrieg.xml"
...
void CopyCharInfoForTrans(MTD_CharInfo* pDest, MMatchCharInfo* pSrcCharInfo, MMatchObject* pSrcObject)
{
	...
	if (pSrcCharInfo)
	{
		...
		pDest->nKills = pSrcCharInfo->m_nTotalKillCount;
		pDest->nDeaths = pSrcCharInfo->m_nTotalDeathCount;
		pDest->nClassJob = pSrcCharInfo->m_ClassJob;
...
bool MMatchServer::LoadInitFile()
{
	...
	if (!MGetActorDefMgr()->ReadXml(NULL, FILENAME_NPC2_XML))
	{
		Log(LOG_PROG, "Read New Quest NPC2 Failed\n");
		return false;
	}

	if (!MGetScenarioMgr()->ReadXml(NULL, FILENAME_SCENARIO2_XML, GetQuest()->GetDropTable()))
	{
		Log(LOG_PROG, "Read New Quest NPC2 Failed\n");
		return false;
	}

	if (!MGetMatchBlitzKrieg()->ReadXml(FILENAME_BLITZKRIEG_XML, NULL))
	{
		Log(LOG_PROG, "Read BlitzKrieg Failed\n");
		return false;
	}
	...
	{
		Log(LOG_PROG, "Read DropGunGame List Failed\n");
		return false;
	}

	if (!MMatchLevelUpReward::InitLevelUpReward())
	{
		Log(LOG_PROG, "Read LevelUpReward List Failed\n");
		return false;
	}

	if (!MGetMatchShop1()->Create(FILENAME_SHOP1))
	{
		Log(LOG_PROG, "Read Shop 1 Item Failed\n");
		return false;
	}

	if (!MGetMatchShop2()->Create(FILENAME_SHOP2))
	{
		Log(LOG_PROG, "Read Shop 2 Item Failed\n");
		return false;
	}

	if (!MGetMatchShop3()->Create(FILENAME_SHOP3))
	{
		Log(LOG_PROG, "Read Shop 3 Item Failed\n");
		return false;
	}

	if (!MGetMatchShop4()->Create(FILENAME_SHOP4))
	{
		Log(LOG_PROG, "Read Shop 4 Item Failed\n");
		return false;
	}
	...
	if (!MGetWarMgr()->ReadXml("warmode.xml")) {
		Log(LOG_PROG, "Load WarMode.xml Failed\n");
		return false;
	}
...
bool MMatchServer::LoadChannelPreset()
{
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!strcmp(szTagName, MTOK_CHANNEL))
		{
...
#endif
			childElement.GetAttribute(&bIsWar, "IsWar");
			...
			if (bIsWar) {
				nChannelType = MCHANNEL_TYPE_WAR;
			}
...
*/
	...
	MGetMatchShop1()->Destroy();
	MGetMatchShop2()->Destroy();
	MGetMatchShop3()->Destroy();
	MGetMatchShop4()->Destroy();
...
#ifdef _SYSTEMUPDATE
	static unsigned long SystemUpdate = nGlobalClock;
	if (nGlobalClock - SystemUpdate > _SYSTEMUPDATETIME)
	{
			GetDBMgr()->UpdateClanRanking();
			GetDBMgr()->UpdateSurvivalRanking();
...
int MMatchServer::ObjectRemove(const MUID& uid, MMatchObjectList::iterator* pNextItor)
{
	...
	if (pObj->GetChatRoomUID() != MUID(0,0))
	{
		...
		if (pRoom)	{ pRoom->RemovePlayer(pObj->GetUID()); }
	}
	...
	for( list<MAsyncJob*>::iterator itAJob = pObj->m_DBJobQ.DBJobQ.begin(); itAJob != pObj->m_DBJobQ.DBJobQ.end(); itAJob = pObj->m_DBJobQ.DBJobQ.begin() )
	{
...
int MMatchServer::ValidateChannelJoin(const MUID& uidPlayer, const MUID& uidChannel)
{
	...
	if(!IsAdminGrade(pObj)) 
	{
		...
		#ifdef _CLOSECHANNEL
		if (!pChannel->IsOpen())
			return MERR_CANNOT_JOIN_CHANNEL;
		#endif
	}
...
#ifdef _CANNOTUSE2TYPE
				if (pTarItem->GetDesc()->m_nWeaponType.Ref() == pItem->GetDesc()->m_nWeaponType.Ref())
				{
					return MERR_CANNOT_EQUIP_EQUAL_ITEM;
				}
#endif
...
bool MMatchServer::CheckItemXML()
{
		...
		itor != MGetMatchItemDescMgr()->end(); ++itor)
	{
		...
		fprintf(fp, "UPDATE Item SET TotalPoint=0, BountyPrice=0, Damage=%d, Delay=%d, Controllability=%d, Magazine=%d, ReloadTime=%d, SlugOutput=%d, Gadget=0, SF=0, FR=0,CR=0,PR=0,LR=0, BlendColor=0, ModelName='', MaxBullet=%d, LimitShield=%d, LimitSpeed=%d, IsCashItem=%d, \n",
			nDamage, nDelay, nControl, nMagazine, nReloadTime, nSlugOutput, nMaxBullet, 
			pItemDesc->m_nLimitShield.Ref(), pItemDesc->m_nLimitSpeed.Ref(), nIsCashItem);
