...
TCHAR g_szDB_INSERT_CONN_LOG[] = _T("{CALL spInsertConnLog (%d, %d, %d, %d, %d)}");

// 게임 로그
//TCHAR g_szDB_INSERT_GAME_LOG[] = _T("{CALL spInsertGameLog ('%s', '%s', '%s', %d, %d, %d, '%s')}");
TCHAR g_szDB_INSERT_GAME_LOG[] = _T("{CALL spInsertGameLog (%d, '%s', '%s')}");

// 게임 플레이 로그
TCHAR g_szDB_INSERT_GAME_PLAYER_LOG[] = _T("{CALL spInsertGamePlayerLog(%d, %d, %d, %d, %d, %d, %d)}");
...
// 서버 로그
TCHAR g_szDB_INSERT_SERVER_LOG[] = _T("{CALL spInsertServerLog (%d, %d, %d)}");
...
TCHAR g_szDB_UPDATE_ACC_CASH[] = _T("UPDATE Account SET Coins=Coins+(%d) WHERE AID=%d");
...
bool MMatchDBMgr::CreateAccount(const TCHAR* szUserID, const TCHAR* szPassword, const int nCert, const TCHAR* szName, const int nAge,  const int nSex)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;	
...
//	if (-1 != rs.GetFieldID("ClassJob"))
//	{
		poutCharInfo->m_ClassJob = (MMatchClassJob) rs.Field("ClassJob").AsInt();
//#ifdef _DEBUG
		mlog("ClassJob : %d\n", poutCharInfo->m_ClassJob);
//#endif
//	}
//	else
//	{
//#ifdef _DEBUG
//		mlog("can't find ClassJob column\n");
//#endif
//		poutCharInfo->m_ClassJob = (MMatchClassJob) 1;
//	}
...
#ifdef _CASHCOINS
	poutAccountInfo->m_nCashCoins	= rs.Field("Coins").AsLong();
#endif
	_STATUS_DB_END(5);
...
bool MMatchDBMgr::BuyBountyItem(const unsigned int nCID, const unsigned int nAID, int nItemID, int nItemCount, int nPrice, const DWORD dwRentHourPeriod, const bool bIsSpendableItem, unsigned long int* poutCIID
#ifdef _EXTRASHOPS
	, MSHOPMODE nShopMode
#endif
	)
{
	...
	try {
		strSQL.Format(g_szDB_BUY_BOUNTY_ITEM, nCID, nAID, nItemID, nItemCount, nPrice, bIsSpendableItem, dwRentHourPeriod
#ifdef _EXTRASHOPS
			, nShopMode
#endif
			);
...
bool MMatchDBMgr::InsertConnLog(const int nAID, const char* szIP, const string& strCountryCode3)
{
	...
	try {
		strSQL.Format( g_szDB_INSERT_CONN_LOG, nAID, vIP[0], vIP[1], vIP[2], vIP[3]);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::UpdateAccCash(const int nAid, const int nCashInc)
{
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_UPDATE_ACC_CASH, nCashInc, nAid);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e) {
...
bool MMatchDBMgr::InsertServerLog(const int nServerID, const int nPlayerCount, const int nGameCount, const DWORD dwBlockCount, const DWORD dwNonBlockCount)
{
	...
	try {		
		strSQL.Format(g_szDB_INSERT_SERVER_LOG, nServerID, nPlayerCount, nGameCount);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
#ifdef _LevelUpReward
bool MMatchDBMgr::InsertLevelUpReward(int nAID, int nCID, int nBounty, int nCash, int nItemID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) 
		return false;
	...
	try 
	{
		strSQL.Format(_T("{CALL spInsertLevelUpReward (%i, %i, %i, %i, %i)}"), nAID, nCID, nBounty, nCash, nItemID);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) 
	{
