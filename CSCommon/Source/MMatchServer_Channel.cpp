...
void CopyChannelPlayerListNodeForTrans(MTD_ChannelPlayerListNode* pDest, MMatchObject* pSrcObject)
{
	...
	pDest->nKills = pSrcObject->GetCharInfo()->m_nTotalKillCount;
	pDest->nDeaths = pSrcObject->GetCharInfo()->m_nTotalDeathCount;
...
bool MMatchServer::ChannelJoin(const MUID& uidPlayer, const MCHANNEL_TYPE nChannelType, const char* pszChannelName)
{
	...
	if (pChannel == NULL)
	{
		...
		if (nChannelType == MCHANNEL_TYPE_WAR) {
			return false;
		}
...
bool MMatchServer::ChannelJoin(const MUID& uidPlayer, const MUID& uidChannel)
{
	...
	if ( MGetServerConfig()->IsUseTicket())
	{
		...
		if ( MGetServerConfig()->GetServerMode() == MSM_MATCH)	{ // 일반 서버일때
	...
	if( pChannel->GetRuleType() != MCHANNEL_RULE_DUELTOURNAMENT && pChannel->GetRuleType() != MCHANNEL_RULE_WAR) {
...
*/
	...
	char szNameProNickPower[255];
	switch (pObj->GetCharInfo()->m_ClassJob)
	{
	case MMCJ_ASSASIN:
		sprintf(szNameProNickPower, "%s(^2%s^9)", pObj->GetCharInfo()->m_szName, "A");
		break;
	case MMCJ_DEFFENDER:
		sprintf(szNameProNickPower, "%s(^2%s^9)", pObj->GetCharInfo()->m_szName, "D");
		break;
	case MMCJ_COWBOY:
		sprintf(szNameProNickPower, "%s(^2%s^9)", pObj->GetCharInfo()->m_szName, "C");
		break;
	case MMCJ_SOLDIER:
		sprintf(szNameProNickPower, "%s(^2%s^9)", pObj->GetCharInfo()->m_szName, "S");
		break;
	default:
		sprintf(szNameProNickPower, "%s(^2%s^9)", pObj->GetCharInfo()->m_szName, "unknown");
		break;
	}
	...
	pCmd->AddParameter(new MCommandParameterString(szNameProNickPower));
...
MMatchChannel* MMatchServer::FindWarChannel() {

	for (auto itor = m_ChannelMap.begin(); itor != m_ChannelMap.end(); itor++) {
		MMatchChannel* pChannel = (*itor).second;
		if (pChannel) {
			if (pChannel->GetChannelType() == MCHANNEL_TYPE_WAR) {
				return pChannel;
			}
		}
	}
...
const MUID MMatchServer::FindFreeChannel(  const MUID& uidPlayer  )
{
	...
	{
		for(map<MUID, MMatchChannel*>::iterator itor=m_ChannelMap.GetTypesChannelMapBegin(MCHANNEL_TYPE_DUELTOURNAMENT); itor!=m_ChannelMap.GetTypesChannelMapEnd(MCHANNEL_TYPE_DUELTOURNAMENT); itor++) {
