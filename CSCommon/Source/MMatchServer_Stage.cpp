...
bool MMatchServer::StageJoin(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	if (pChannel->GetChannelType() == MCHANNEL_TYPE_WAR) {
		MMatchClassJob nClass = pObj->GetCharInfo()->GetClassJob();

		if (nClass == MMCJ_ASSASIN || nClass == MMCJ_COWBOY) {
			pObj->SetTeam(MMT_RED);
		}
		else {
			pObj->SetTeam(MMT_BLUE);
	...
	// Cast Join
	MCommand* pNew = nullptr;
	if (pChannel->GetChannelType() == MCHANNEL_TYPE_WAR) {
		pNew = new MCommand(m_CommandManager.GetCommandDescByID(MC_WAR_STAGE_JOIN), MUID(0, 0), m_This);
	}
	else {
		pNew = new MCommand(m_CommandManager.GetCommandDescByID(MC_MATCH_STAGE_JOIN), MUID(0, 0), m_This);
	}
...
void MMatchServer::OnStageCreate(const MUID& uidChar, char* pszStageName, bool bPrivate, char* pszStagePassword)
{
	...
	if ( (MGetServerConfig()->GetServerMode() == MSM_CLAN && pChannel->GetChannelType() == MCHANNEL_TYPE_CLAN) || pChannel->GetChannelType() == MCHANNEL_TYPE_DUELTOURNAMENT || pChannel->GetChannelType() == MCHANNEL_TYPE_WAR) {
		return;
	}
	
	MUID uidStage;

	if (!IsAdminGrade(pObj) && pChannel->IsEvent())
	{
...
void MMatchServer::OnStageSetting(const MUID& uidPlayer, const MUID& uidStage, void* pStageBlob, int nStageCount)
{
	...
	if ((pChannel) && (bCheckChannelRule))
	{
		...
		if (pRule)
		{
			if (pNode->nGameType != MMATCH_GAMETYPE_BLITZKRIEG) {
	...
	if (MGetGameTypeMgr()->IsQuestDerived(pNode->nGameType))
	{
		if (pNode->bForcedEntryEnabled == true) {
	...
	if (nLastGameType != pSetting->GetGameType())
	{
		...
		{
			OnStageMap(uidStage, (char*)MGetScenarioMgr()->GetDefaultScenarioName());
		}
...
void MMatchServer::OnDropGunGameRequestItem(const MUID& uidPlayer)
{
	...
	if (pStage->GetStageSetting())
	{
		if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_DROPGUNGAME)
		{
					   ...
					   int* poutAttackerExp, int* poutVictimExp)
{
	...
	else if (nGameType == MMATCH_GAMETYPE_TRAINING)
	{
		*poutAttackerExp = 0;
		*poutVictimExp = 0;
		return;
