...
//			P(MPT_VECTOR, "Position")
	...
	if(nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT || nSharedType&MSCT_AGENT) {
		...
		if (nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT) {
			...
			// Machine To Machine
#ifdef _CLOSECHANNEL
			C(MC_ADMIN_OPENCLOSE_CHANNEL, "MC_ADMIN_OPENCLOSE_CHANNEL", "Open or close a channel", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChannel")
			P(MPT_BOOL, "isOpen")
#endif
			// No-Leadshot Request
			C(MC_MATCH_NOLEAD, "Match.Gunz.AntiLead", "AntiLead Information", MCDT_PEER2PEER)
				P(MPT_BLOB, "ShotInfo");

			/////////////////////////////////////////
			// Peer To Peer
			C(MC_PEER_DAMAGECOUNTER, "Match.Gunz.DamageCounter", "Damage counter information", MCDT_PEER2PEER)
				P(MPT_INT, "Damage");
			...
			C(MC_PEER_KILLSTREAK, "Killstreak", "Killstreak sounds and message.", MCDT_PEER2PEER)
				...
				P(MPT_INT, "nOldKillStreakCount")
				P(MPT_STR, "szName2");

			C(MC_PEER_PAYBACK, "Payback", "Payback sounds and message.", MCDT_PEER2PEER)
				P(MPT_STR, "szName")
				P(MPT_STR, "szName2");

			C(MC_PEER_VAMPIREHEALT, "Vampire Healt", "Update Attacker Healt", MCDT_PEER2PEER)
				P(MPT_UID, "AttackerUID");

			C(MC_PEER_HPAP, "", "", MCDT_PEER2PEER)
				P(MPT_UID, "VictimUID")
				P(MPT_INT, "HP")
				P(MPT_INT, "AP");

			C(MC_PEER_VAMPIRE_DEATH, "", "", MCDT_PEER2PEER)
				P(MPT_UID, "AttackerUID")
				P(MPT_INT, "HP")
				P(MPT_INT, "AP");
			...
			// blitzkrieg.
			C(MC_BLITZ_NPC_SPAWN, "Match.Blitz.Player NPC Spawn", "BlitzKrieg", MCDT_MACHINE2MACHINE)
				P(MPT_UID,		"udiOwner")
				P(MPT_UID,		"uidActor")
				P(MPT_STR,		"szActorName")
				P(MPT_UCHAR,	"nTeam")
				P(MPT_UCHAR,	"nPosIndex")
				P(MPT_USHORT,	"nHP")
				P(MPT_USHORT,	"nAP")
				P(MPT_FLOAT,	"fUnknown")


			// WarMode
			C(MC_WAR_SPAWN_TOWER, "Match.War.NPC Spawn", "WarMode", MCDT_MACHINE2MACHINE)
				P(MPT_UID,		"udiOwner")
				P(MPT_STR,		"szActorName")
				P(MPT_UID,		"uidActor")
				P(MPT_UCHAR,	"nTeam")
			...
			C(MC_WAR_DUMMY_BASICINFO, "Match.Dummy.BasicInfo", "DUMMY BasicInfo", MCDT_PEER2PEER)
				P(MPT_BLOB, "Info")

			C(MC_WAR_GAME_REQUEST_JOINGAME, "", "", MCDT_MACHINE2MACHINE)

			C(MC_WAR_STAGE_JOIN, "", "", MCDT_MACHINE2MACHINE)
				P(MPT_UID, "uidPlayer")
				P(MPT_UID, "uidStage")
				P(MPT_UINT, "nRoomNo")
				P(MPT_STR, "StageName");

			C(MC_WAR_NPC_DEAD, "", "", MCDT_MACHINE2MACHINE)
				P(MPT_UID, "uidPlayer")
				P(MPT_UID, "uidNPC")

			C(MC_WAR_SYNC_HPAP, "", "", MCDT_MACHINE2MACHINE)
				P(MPT_UID, "uidNpc")
				P(MPT_INT, "nHP")
				P(MPT_INT, "nAP")
		}		
