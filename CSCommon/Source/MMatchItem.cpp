...
MMatchItemDesc::MMatchItemDesc() : m_nID(0), m_nSlot(MMIST_NONE), m_pEffect(NULL), m_bSlugOutput(0),m_nColor(0xFFFFFFFF), m_nImageID(0), m_nBulletImageID(0), m_nMagazineImageID(0), m_bIsCashItem(false), m_bIsSpendableItem(false)
{
	...
	m_nLimitShield.Set_MakeCrc(100);
...
void MMatchItemDesc::CacheCRC32( MMatchCRC32XORCache& crc )
{
	...
	crc.CRC32XOR( m_nLimitShield.Ref());
...
void MMatchItemDesc::ShiftFugitiveValues()
{
	...
	m_nLimitShield.ShiftHeapPos_CheckCrc();
...
void MMatchItemDesc::DumpBinary(FILE* fp)
{
	...
	fwrite(&m_nLimitShield.Ref(), sizeof(int), 1, fp);
...
void MMatchItemDesc::LoadBinary(FILE* fp)
{
	...
	fread(&m_nLimitShield.Ref(), sizeof(int), 1, fp);				m_nLimitShield.MakeCrc();
...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	pNewDesc->m_nLimitShield.Set_MakeCrc(100);
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		else if (!stricmp(szAttrName, MICTOK_WEAPON))
		{
			...
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_BOWS))			pNewDesc->m_nWeaponType.Set(MWT_BOWS);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_KATANA1))			pNewDesc->m_nWeaponType.Set(MWT_KATANA1);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_KATANA2))			pNewDesc->m_nWeaponType.Set(MWT_KATANA2);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_KATANA3))			pNewDesc->m_nWeaponType.Set(MWT_KATANA3);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_KATANA4))			pNewDesc->m_nWeaponType.Set(MWT_KATANA4);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_AXE))			pNewDesc->m_nWeaponType.Set(MWT_AXE);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_BASEBALL))			pNewDesc->m_nWeaponType.Set(MWT_BASEBALL);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_FIGHTER))			pNewDesc->m_nWeaponType.Set(MWT_FIGHTER);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_SWORD2))			pNewDesc->m_nWeaponType.Set(MWT_SWORD2);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_DOUBLEKATANA2))			pNewDesc->m_nWeaponType.Set(MWT_DOUBLE_KATANA2);
			else if (!stricmp(szAttrValue, MICTOK_WEAPON_DOUBLEKATANA3))		pNewDesc->m_nWeaponType.Set(MWT_DOUBLE_KATANA3);
		...
		else if (!stricmp(szAttrName, MICTOK_LIMITSHIELD))
		{
			pNewDesc->m_nLimitShield.Set(atoi(szAttrValue));
			pNewDesc->m_nLimitShield.MakeCrc();
		}
...
MMatchWeaponType GetWeaponType(MMatchMeleeItemType nMeleeItemType)
{
	...
	case MIT_KATANA1:		return MWT_KATANA1;
	case MIT_KATANA2:		return MWT_KATANA2;
	case MIT_KATANA3:		return MWT_KATANA3;
	case MIT_KATANA4:		return MWT_KATANA4;
	case MIT_AXE:			return MWT_AXE;
	case MIT_BASEBALL:		return MWT_BASEBALL;
	case MIT_FIGHTER:		return MWT_FIGHTER;
	case MIT_SWORD2:		return MWT_SWORD2;
	case MIT_DOUBLE_KATANA2:	return MWT_DOUBLE_KATANA2;
	case MIT_DOUBLE_KATANA3:	return MWT_DOUBLE_KATANA3;
	default:
		// 없는 타입
		_ASSERT(0);
	}
