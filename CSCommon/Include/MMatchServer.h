...
protected:
	...
	#ifdef _CLOSECHANNEL
	void OnAdminOpenCloseChannel(const MUID& uidAdmin, const MUID uidChannel, const bool bIsOpen);
	#endif
...
public:
	bool LaunchWarMatch(MWarEvent pEvent);
	void ResponseWarJoinGame(MUID uid);
	void RequestWarNPCDead(MUID uidAttacker, MUID uidNPC);
	void UpdateWarNPCHpAp(MUID uidOwner, MUID uidNpc, int nHP, int nAP);
