...
public:
	...
	bool UpdateAccCash(const int nAid, const int nCashInc);
	...
	bool BuyBountyItem(const unsigned int nCID, const unsigned int nAID, int nItemID, int nItemCount, int nPrice, const DWORD dwRentHourPeriod, const bool bIsSpendableItem, unsigned long int* poutCIID, MSHOPMODE nShopMode);
	...
	bool InsertLevelUpReward(int nAID, int nCID, int nBounty, int nCash, int nItemID);
#endif
