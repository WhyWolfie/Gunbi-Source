...
void MBMatchServer::ResponseShopItemList(const MUID& uidPlayer, const int nFirstItemIndex, const int nItemCount)
{
	...
	pNew->AddParameter( MakeShopItemListBlob(nFirstItemIndex, nItemCount, uidPlayer) );
...
MCommandParameterBlob* MBMatchServer::MakeShopItemListBlob( const int nFirstItemIndex, const int nItemCount, const MUID& uidPlayer) const
{
	...
	if (pObj->GetCharInfo()->m_ClassJob == MMCJ_ASSASIN)
	{
		int nRealItemCount = nItemCount;

		if ((nItemCount <= 0) || (nItemCount > MGetMatchShop1()->GetCount())) 
		{
			nRealItemCount = MGetMatchShop1()->GetCount();
		}
		...
		for (int i = nFirstItemIndex; i < nFirstItemIndex + nRealItemCount; i++)
		{
			...
			ShopItemNode* pSINode = MGetMatchShop1()->GetSellItemByIndex(i);
	...
	else if (pObj->GetCharInfo()->m_ClassJob == MMCJ_COWBOY)
	{
		int nRealItemCount = nItemCount;

		if ((nItemCount <= 0) || (nItemCount > MGetMatchShop2()->GetCount()))
		{
			nRealItemCount = MGetMatchShop2()->GetCount();
		}
		...
		{
			MTD_ShopItemInfo *pInfo = (MTD_ShopItemInfo *) MGetBlobArrayElement(pItemArray, nIndex++);
			ShopItemNode* pSINode = MGetMatchShop2()->GetSellItemByIndex(i);
	...
	else if (pObj->GetCharInfo()->m_ClassJob == MMCJ_DEFFENDER)
	{
		int nRealItemCount = nItemCount;

		if ((nItemCount <= 0) || (nItemCount > MGetMatchShop3()->GetCount()))
		{
			nRealItemCount = MGetMatchShop3()->GetCount();
		}
		...
		{
			MTD_ShopItemInfo *pInfo = (MTD_ShopItemInfo *) MGetBlobArrayElement(pItemArray, nIndex++);
			ShopItemNode* pSINode = MGetMatchShop3()->GetSellItemByIndex(i);
	...
	else if (pObj->GetCharInfo()->m_ClassJob == MMCJ_SOLDIER)
	{
		int nRealItemCount = nItemCount;

		if ((nItemCount <= 0) || (nItemCount > MGetMatchShop4()->GetCount()))
		{
			nRealItemCount = MGetMatchShop4()->GetCount();
		}
		...
		{
			MTD_ShopItemInfo *pInfo = (MTD_ShopItemInfo *) MGetBlobArrayElement(pItemArray, nIndex++);
			ShopItemNode* pSINode = MGetMatchShop4()->GetSellItemByIndex(i);
...
void MBMatchServer::OnRequestBuyItem(const MUID& uidPlayer, const unsigned long int nItemID, const int nItemCount)
{
	...
	if( MGetMatchShop1()->IsSellItem(nItemID) && pObj->GetCharInfo()->m_ClassJob == MMCJ_ASSASIN)
	{
	...
	else if( MGetMatchShop2()->IsSellItem(nItemID) && pObj->GetCharInfo()->m_ClassJob == MMCJ_COWBOY)
	{
		MMatchItemDesc* pItemDesc = MGetMatchItemDescMgr()->GetItemDesc( nItemID );
		if (pItemDesc == NULL) return;
	...
	else if( MGetMatchShop3()->IsSellItem(nItemID) && pObj->GetCharInfo()->m_ClassJob == MMCJ_DEFFENDER)
	{
		MMatchItemDesc* pItemDesc = MGetMatchItemDescMgr()->GetItemDesc( nItemID );
		if (pItemDesc == NULL) return;
	...
	else if( MGetMatchShop4()->IsSellItem(nItemID) && pObj->GetCharInfo()->m_ClassJob == MMCJ_SOLDIER)
	{
		MMatchItemDesc* pItemDesc = MGetMatchItemDescMgr()->GetItemDesc( nItemID );
		if (pItemDesc == NULL) return;
	...
	if( !CheckUserCanBuyItem(pObj, nItemID, nItemCount, dwPrice 
		#ifdef _EXTRASHOPS
		, nShopMode
		#endif
		) ) { return; }
	...
	)
{
	...
	#ifdef _EXTRASHOPS
	bool bIsCashItem = (nShopMode == SHOP_CASH);
	#endif
	...
	#ifdef _EXTRASHOPS
	if (nShopMode == SHOP_CASH)
	{
		if (static_cast<DWORD>(pAccountInfo->m_nCashCoins) < dwPrice) 
		{
			MCommand* pNew = CreateCommand( MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID() );
...
#ifdef _CASHCOINS
	pNew->AddParameter(new MCommandParameterInt(pObj->GetAccountInfo()->m_nCashCoins));
#endif
