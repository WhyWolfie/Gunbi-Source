...
#define _EXTENDED_CHAT_COLORS 1
...
struct ChatColor {
	...
	int nColors;
	queue<unsigned long> colors;
};

class ChatColorManager : public map<int, ChatColor> {
public:
	void AddColor(int key, ChatColorMode mode, unsigned long count, ...) {
		if (count < 1) return;
		va_list vl;
		va_start(vl, count);

		ChatColor chatColor;
		chatColor.colorMode = mode;
		chatColor.nColors = count;

		for(int i = 0; i < count; ++i)
			chatColor.colors.push(va_arg(vl, unsigned long));
		va_end(vl);

		insert(pair<int, ChatColor>(key, chatColor));		
...
void InitColorMap()
{
#define CHAT_COLOR(K, M, C, ...) chatColorManager.AddColor((K), (M), (C), ##__VA_ARGS__)

	//Default GunZ Colours
	CHAT_COLOR(48, CHAT_COLOR_NORMAL, 1, 0xFF808080);
	CHAT_COLOR(49, CHAT_COLOR_NORMAL, 1, 0xFFFF0000);
	CHAT_COLOR(50, CHAT_COLOR_NORMAL, 1, 0xFF00FF00);
	CHAT_COLOR(51, CHAT_COLOR_NORMAL, 1, 0xFF0000FF);
	CHAT_COLOR(52, CHAT_COLOR_NORMAL, 1, 0xFFFFFF00);
	CHAT_COLOR(53, CHAT_COLOR_NORMAL, 1, 0xFF800000);
	CHAT_COLOR(54, CHAT_COLOR_NORMAL, 1, 0xFF008000);
	CHAT_COLOR(55, CHAT_COLOR_NORMAL, 1, 0xFF000080);
	CHAT_COLOR(56, CHAT_COLOR_NORMAL, 1, 0xFF808000);
	CHAT_COLOR(57, CHAT_COLOR_NORMAL, 1, 0xFFFFFFFF);

	//Colour wheel generated
	CHAT_COLOR(65, CHAT_COLOR_NORMAL, 1, 0XFFFF8080);
	CHAT_COLOR(66, CHAT_COLOR_NORMAL, 1, 0XFFFF0000);
	CHAT_COLOR(67, CHAT_COLOR_NORMAL, 1, 0XFFFFBB80);
	CHAT_COLOR(68, CHAT_COLOR_NORMAL, 1, 0XFFFF7600);
	CHAT_COLOR(69, CHAT_COLOR_NORMAL, 1, 0XFFFFF680);
	CHAT_COLOR(70, CHAT_COLOR_NORMAL, 1, 0XFFFFEC00);
	CHAT_COLOR(71, CHAT_COLOR_NORMAL, 1, 0XFFCEFF80);
	CHAT_COLOR(72, CHAT_COLOR_NORMAL, 1, 0XFF9DFF00);
	CHAT_COLOR(73, CHAT_COLOR_NORMAL, 1, 0XFF93FF80);
	CHAT_COLOR(74, CHAT_COLOR_NORMAL, 1, 0XFF27FF00);
	CHAT_COLOR(75, CHAT_COLOR_NORMAL, 1, 0XFF80FFA7);
	CHAT_COLOR(76, CHAT_COLOR_NORMAL, 1, 0XFF00FF4E);
	CHAT_COLOR(77, CHAT_COLOR_NORMAL, 1, 0XFF80FFE2);
	CHAT_COLOR(78, CHAT_COLOR_NORMAL, 1, 0XFF00FFC4);
	CHAT_COLOR(79, CHAT_COLOR_NORMAL, 1, 0XFF80E2FF);
	CHAT_COLOR(80, CHAT_COLOR_NORMAL, 1, 0XFF00C4FF);
	CHAT_COLOR(81, CHAT_COLOR_NORMAL, 1, 0XFF80A7FF);
	CHAT_COLOR(82, CHAT_COLOR_NORMAL, 1, 0XFF004EFF);
	CHAT_COLOR(83, CHAT_COLOR_NORMAL, 1, 0XFF9380FF);
	CHAT_COLOR(84, CHAT_COLOR_NORMAL, 1, 0XFF2700FF);
	CHAT_COLOR(85, CHAT_COLOR_NORMAL, 1, 0XFFCE80FF);
	CHAT_COLOR(86, CHAT_COLOR_NORMAL, 1, 0XFF9D00FF);
	CHAT_COLOR(87, CHAT_COLOR_NORMAL, 1, 0XFFFF80F6);
	CHAT_COLOR(88, CHAT_COLOR_NORMAL, 1, 0XFFFF00EC);
	CHAT_COLOR(89, CHAT_COLOR_NORMAL, 1, 0XFFFF80BB);
	CHAT_COLOR(90, CHAT_COLOR_NORMAL, 1, 0XFFFF0076);

	//Flashing Colours
	CHAT_COLOR(124, CHAT_COLOR_FLASH, 2, 0xFFFFFFFF, 0xFF000000);
}

void UpdateFlashColors() {
	static DWORD prevTime = 0;
	if (prevTime == 0) prevTime = timeGetTime();
	DWORD time = timeGetTime();

	if (time - prevTime > 1000) {
		for (auto it = chatColorManager.begin(); it != chatColorManager.end(); it++) {
			if (it->second.colorMode == CHAT_COLOR_FLASH) {
...
unsigned long int MMGetColor(int c) {
	if (chatColorManager.empty()) InitColorMap();
	UpdateFlashColors();

	auto it = chatColorManager.find(c);

	if (it == chatColorManager.end())
		return 0xFFFFFFFF;

	ChatColor& chatColor = it->second;
	...
	while(true){
		...
		if(nPos + 1 < strlen(pSrc)){
			...
			if(TestDigit(pSrc[nPos+1])){
				SetColor(MCOLOR(MMGetColor(pSrc[nPos+1])));
				pSrc = pSrc + nPos + 2;
			} else {
				Text(x+nOffset, y, "^");
...
#define		MAX_WIDGET_LINE_LENGTH	1024
	...
	for(i; i<nStrLen; i++)
	{
		...
		#else
		if(c=='^' && (TestDigit(szText[i+1])))
		#endif
		{
			SetColor(MCOLOR(MMGetColor(szText[i+1])));
			i++;
			continue;
		}
