...
//		pMeshNode->ConnectToNameID();//이름을 ID 로 바꾼다..
		...
		else if (NCMPNAME("eq_wl_bows", 9)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_left_bows;
		}
		...
		else if( NCMPNAME("eq_wd_katana1",12) ) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_katana1;
		}
		else if (NCMPNAME("eq_wd_katana2", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_katana2;
		}
		else if (NCMPNAME("eq_wd_katana3", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_katana3;
		}
		else if (NCMPNAME("eq_wd_katana4", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_katana4;
		}
		else if (NCMPNAME("eq_wd_axe", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_axe;
		}
		else if (NCMPNAME("eq_wd_baseball", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_baseball;
		}
		else if (NCMPNAME("eq_wd_fighter", 12)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_fighter;
		}
		else if (NCMPNAME("eq_wl_sword2", 11)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_sword;
		}
		else if (NCMPNAME("eq_wr_blade2", 11)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_blade2;
		}
		else if (NCMPNAME("eq_wl_blade2", 11)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_left_blade2;
		}
		else if (NCMPNAME("eq_wr_blade3", 11)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_blade3;
		}
		else if (NCMPNAME("eq_wl_blade3", 11)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_left_blade3;
		}
	} 
...
#define MZF_READ_XOR_SPECIAL(x,y) { if(!mzf->Read((x),(y))) return false; for (int i = 0; i < y; ++i) *(BYTE*)(x + i) ^= 0; }
  ...
  if(count_mtrl)
  {
	  for(i=0;i<count_mtrl;i++) {
		...
		node->m_bTwoSided = twoside != 0;

		char additive = 0;
		MZF_READ_XOR(&additive, sizeof(char));
		//node->m_bAdditive = additive != 0'
...
#define MZF_READ(x,y) { if(!mzf->Read((x),(y))) return false; }
	...
	for(i=0;i<m_phd_t->mtrl_num;i++) {
		...
		if(m_phd_t->ver > EXPORTER_MESH_VER2) {//ver3 부터
			...
			node->m_bTwoSided = twoside != false;
		}

		if(m_phd_t->ver > EXPORTER_MESH_VER4) {
			int additive = 0;
			MZF_READ(&additive, sizeof(int));
			node->m_bAdditive = additive != false;
		}
		...
		if(m_phd_t->ver > EXPORTER_MESH_VER7 )//ver8 부터
		{
			...
			node->m_bAlphaTestMap = alpha_test != false;
			node->m_nAlphaTestValue = alpha_test;
		}
