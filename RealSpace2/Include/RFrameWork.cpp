...
#include "RFont.h"
#include "RMeshUtil.h"
#include "RFrameWork.h"	// Full-screen toggle.
...
void RFrame_SetFullScreen(bool bFullScreen)	{	// Full-screen toggle.
	RMODEPARAMS ModeParams;

	ModeParams.nWidth = RGetScreenWidth();
	ModeParams.nHeight = RGetScreenHeight();
	ModeParams.PixelFormat = RGetPixelFormat();

	ModeParams.bFullScreen = bFullScreen;

	RResetDevice(&ModeParams);

	RFrame_AdjustWindowPosition(ModeParams.nWidth, ModeParams.nHeight, ModeParams.bFullScreen);
}

void RFrame_AdjustWindowPosition(int nWidth, int nHeight, bool bFullScreen)	{	// Full-screen toggle.
	RECT WindowRect, DesktopWindowRect;
	DWORD dwStyle = (bFullScreen == true) ? WS_POPUP | WS_SYSMENU : WS_POPUP | WS_CAPTION | WS_SYSMENU;

	GetWindowRect(GetDesktopWindow(), &DesktopWindowRect);

	SetRect(&WindowRect, 0, 0, nWidth, nHeight);
	AdjustWindowRect(&WindowRect, dwStyle, FALSE);
	MoveWindow(g_hWnd, (DesktopWindowRect.right - (WindowRect.right - WindowRect.left)) / 2,
		(DesktopWindowRect.bottom - (WindowRect.bottom - WindowRect.top)) / 2,
...
int RMain(const char *AppName, HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow, RMODEPARAMS *pModeParams, WNDPROC winproc, WORD nIconResID )
{
	...
	RFrame_AdjustWindowPosition(pModeParams->nWidth, pModeParams->nHeight, pModeParams->bFullScreen);
	// Full-screen toggle. End.
