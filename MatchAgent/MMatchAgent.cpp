...
#define MINTERVAL_GARBAGE_SESSION_PING	(3 * 60 * 1000)	// 3 min
	...
	if (nGlobalClock - tmLastGarbageSessionCleaning > MINTERVAL_GARBAGE_SESSION_PING)
	{
		...
		LOG(LOG_PROG, "GARBAGE SESSION CLEANING : ClientCount=%d, StageCount=%d, SessionCount=%d, CommandPool=%d\n", 
			GetClientCount(), GetStageCount(), GetCommObjCount(), CMemPool<MCommand>::GetCapacity());
	}
	...
	case MSGID_RAWCOMMAND:
		{
			...
			{
				delete pCmd;
				// 핵방어를 해서 위조커맨드가 여기서 많이 걸리게 되었다. 그러나 그 때문에 로그가 너무 폭발적이라서 일단 급하게 생략함
				//mlog( "Fail ParseUDPPacket.\n" );
				return;
			}

			// Code의 가독성 및 효율성을 위하여
			// 가장 흔하게 들어오는 Packet은 MC_UDP_PING이다. -> 효율성
			//
			// 아래의 else if들과 동작 방식이 다르다. -> 가독성
			// 아래의 else if들은 command를 삭제하지 않는다.
